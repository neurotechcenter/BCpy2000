...
def packnode(node, lut=None):
    ...
    for key, val in list(node.children.items()):
        ...
        if len(key) > packedsize_key:
            raise TrieError(
                "encoded key '%s' overflows the number of bytes allocated for keys (%d)"
                % (key, packedsize_key)
            )
        vals.extend(key)
...
def unpacknode(x, ind=0):
    node = trienode()
    nchildren, node.parent, node.count = list(
        map(int, struct.unpack(packedfmt_header, x[ind : ind + packedsize_header]))
    )
...
class trie:
    ...
    def __init__(self, packed=None):
        if packed == None:
            self.nodes = [trienode(parent=0)]
        else:
            self.nodes = packedtriedata(packed)
            try:
                self.nodes[0].children.get("a")
            except:
                # Nasty heuristic to determine whether raw string was intended to be a filename or raw binary content
                if len(packed) < 1024 and len(os.path.basename(packed)) < 64:
                    raise IOError("file %s not found" % packed)
                else:
                    raise TrieError(
                        "invalid content in <%s instance at 0x%08x>"
                        % (self.nodes.__class__.__name__, id(self.nodes))
                    )
    ...
    def add(self, string):
        if self.ispacked():
            raise TrieError("cannot add strings to a packed trie")
        ...
        while ind != None:
            ...
            if parent == ind:
                ind = None  # stop after dealing with node 0, which is its own parent
            else:
                ind = parent
    ...
    def dist(self, node, normalize=False):
        ...
        for letter, ind in list(node.children.items()):
            ...
            if normalize:
                distrib[letter] /= float(node.count)
        return distrib
    ...
    def gethash(self, length=16):
        if not self.ispacked():
            raise TrieError("not packed")
        if self.isondisk():
            raise TrieError("cannot hash: not in memory")
        m = md5.md5()
    ...
    def write(self, filestem="", suffix=None, extension=".trie"):
        if not self.ispacked():
            raise TrieError("not packed")
        if self.isondisk():
            raise TrieError("already on disk")
        if suffix == None:
    ...
    def read(self):
        if not self.isondisk():
            raise TrieError("already in memory")
...
def packtrie(t):
    ...
    """  ###
    if t.ispacked():
        raise TrieError("already packed")
    x = ""
    lut = [0]
    for node in t.nodes[:-1]:
        lut.append(lut[-1] + node.packed_size())
    if progress != None:
        pr = progress(len(t.nodes), " packing")
    for i in range(len(t.nodes)):
        x += packnode(t.nodes[i], lut=lut)
        if i % 5000 == 0 and progress != None:
            pr.update(i, "(%d nodes)" % i)
    if progress != None:
        pr.done()
    return x
...
class packedtriedata:
    ...
    def __repr__(self):
        ...
        if isinstance(self.content, str):
            s += "\n    %.3g MB in memory (packed)" % (
                float(len(self.content)) / 1024**2
            )
        elif isinstance(self.content, filestr):
