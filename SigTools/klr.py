...
def klr_cg(K, Y, C=0, **kwargs):
    ...
    if len(unrec):
        raise TypeError(
            "unrecognized option(s): %s" % ", ".join(['"' + a + '"' for a in unrec])
        )
    ...
    if dtype == numpy.float32:
        ...
        raise ValueError("unrecognized numeric type")
    N = K.shape[1]
    ...
    if opts.maxEval == None:
        opts.maxEval = (Y.flat != 0).sum() * 5
    ...
    if opts.ridge.size == 1:
        if opts.ridge > 0.0:
            opts.ridge *= meanabsdiagK  # make the ridge relative to the max eigen-value
        else:
            opts.ridge *= -1.0  # negative value means absolute ridge
    ...
    def vcat(a, b):
        return numpy.r_[numpy.asmatrix(a), numpy.asmatrix(b)]

    def scalarproduct(a, b):
        return numpy.inner(a.flat, b.flat)
    ...
    if opts.verb > 0:  # debug code
        if opts.verb > 1:
            ...
            lend = "\r"
        out.write(
            "%3d) %3d x=[%8f,%8f,.] J=%5f (%8f+%8f) |dJ|=%8g\n"
    ...
    while iter < min(opts.maxIter, 2e6):
        ...
        if iter == 1 and (r2 <= opts.tol or r2 <= eps):
            break
        ...
        ostep = numpy.inf
        step = tstep
        # max(tstep,abs(1e-6/dtdJ)); # prev step size is first guess!
        odtdJ = dtdJ
        # one step before is same as current
        wb.flat = (wb + step * d).flat
        a = K * d[:-1]
        Kd = vcat(
            K * d[:-1] + numpy.multiply(opts.ridge, d[:-1]).sum(), bPC * d[-1]
        )  # cache, so don't comp dJ
        ...
        while j < opts.maxLineSrch:
            ...
            if (
                abs(dtdJ) < opts.tol
                or abs(dtdJ * step) < eps
                or abs(odtdJ - dtdJ) < eps
            ) and (iter > 1 or j >= 3):
                break  # Ensure we do decent line search for 1st step size!

            if opts.verb > 1:
                Ed = -scalarproduct(
                    numpy.log(g), numpy.multiply(Y, wghtY)
                )  # P(D|w,b,fp)
            ...
            if oodtdJ * odtdJ < 0 < odtdJ * dtdJ and abs(
                step * odtdJ / (odtdJ - dtdJ)
            ) > abs(
                ostep
            ):  # oodtdJ still brackets, and would jump outside
                step = ostep + step
                # make as if we jumped here directly.
                odtdJ = oodtdJ
            ostep = step
        ...
        if opts.verb > 0:  # debug code
            out.write(
                ("%3d) %3d x=[%8f,%8f,.] J=%5f (%8f+%8f) |dJ|=%8g" + lend)
        ...
        if iter == 1:
            ...
            madJ = madJ * (1 - opts.marate) + (oJ - J) * (opts.marate)
            # move-ave objective grad est

        if (
            r2 <= max(opts.tol, eps, r02 * opts.tol0)
            or neval > opts.maxEval
            or madJ < opts.objTol
            or madJ / dJ0 < opts.objTol0
        ):
            break  # term tests

        # ------------------------------------------------
        # conjugate direction selection
        delta = max(0, scalarproduct(Mr - oMr, -dJ / or2))
        # Polak-Ribier
