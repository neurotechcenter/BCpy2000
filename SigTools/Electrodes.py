...
class channel:
    def __init__(self, pos, neg, gnd, label=None):
        def cleanup(s):
            if isinstance(s, (tuple, list)):
                s = reduce(list.__add__, [list(cleanup(x)) for x in s])
            if isinstance(s, str):
                s = s.replace(",", " ").lstrip("\r\n ([{").rstrip("}]) \n\r")
                s = s.replace("+", " ").replace("&", " ")
                s = s.split()
        ...
        if isinstance(pos, (tuple, list)):
            pos = [(k, 1.0 / len(pos)) for k in pos]
        self.pos = dict(pos)

        neg = cleanup(neg)
        if isinstance(neg, (tuple, list)):
            neg = [(k, 1.0 / len(neg)) for k in neg]
        self.neg = dict(neg)
    ...
    def _update(self, add=(), subtract=()):
        ...
        if len(self.pos) and len(self.neg):
            ...
            if (
                max(
                    [abs(v - carred) for v in list(self.pos.values())]
                    + [abs(v - car) for v in list(self.neg.values())]
                )
                < tol
            ):
                for k in self.pos:
                ...
                max(
                    [abs(v - carred) for v in list(self.neg.values())]
                    + [abs(v - car) for v in list(self.pos.values())]
                )
                < tol
            ):
                for k in self.pos:
    ...
    def _dict2str(self, d):
        ...
        if len(d) > 4:
            if avg:
                s = "(average of %d electrodes)" % len(d)
            else:
                s = "(weighted combination of %d electrodes)" % len(d)
        elif avg:
            s = "(" + " + ".join(k) + ")/%d" % len(d)
        else:
            ...
            for i in range(len(w)):
                if abs(w[i] - 1.0) < tol:
                    pass
                elif abs(1.0 / w[i] - round(1.0 / w[i])) < tol:
                    k[i] = "%s/%d" % (k[i], round(1.0 / w[i]))
                else:
                    k[i] = "%g*%s" % (w[i], k[i])
            s = " + ".join(k)
            if len(d) > 1:
    ...
    def __sub__(self, x):
        return self.copy().__isub__(x)

    def __mul__(self, x):
        return self.copy().__imul__(x)

    def __rmul__(self, x):
        return self.copy().__imul__(x)

    def __div__(self, x):
        return self.copy().__idiv__(x)
...
class ChannelSet(numpy.matrix):
    ...
    def __array_finalize__(self, obj):
        ...
        super(ChannelSet, self).__array_finalize__(
            obj
        )  # since we're subclassing a subclass, we must be careful to run the __array_finalize__ method for the intermediate class

    def plot(self, *pargs, **kwargs):
        return_handles = kwargs.pop("return_handles", False)
        if "coords" not in kwargs:
            kwargs["coords"] = self.__coords
        if self.__plotopts:
            kwargs.update(self.__plotopts)
    ...
    def stplot(self, img, fs=None, drawnow=True, **kwargs):
        ...
        x = None
        pylab.subplot(121)
        h = SigTools.Plotting.imagesc(
            img, x=x, aspect="auto", drawnow=False, picker=5, **kwargs
        )
        ax = pylab.gca()
    ...
    def get_labels(self, lower=False, flatten=True, assert_unique=False):
        ...
        for row in self.A:
            ...
            flat += row
            nonflat.append(row)
        if assert_unique and len(set([x.lower() for x in flat])) < len(flat):
            raise ValueError("duplicate channel labels in ChannelSet")
        if flatten:
            return flat
        else:
            return nonflat
        ...
        for label in labels:
            if isinstance(label, int):
                ...
                if label.lower() in candidates:
                    label = candidates.index(label.lower())
                else:
                    label = None
            out.append(label)

        if error_if_not not in (None, False):
            if isinstance(error_if_not, str):
                suffixstr = error_if_not
            else:
                suffixstr = ""
            notfound = [str(label) for label, ind in zip(labels, out) if ind is None]
            if len(notfound) == 1:
                raise ValueError(
                    "channel %s was not found%s" % (notfound[0], suffixstr)
                )
            elif len(notfound):
                raise ValueError(
                    "the following channels were not found%s: %s"
                    % (suffixstr, " ".join(notfound))
                )

        if dict_output:
            out = dict([(str(k), v) for k, v in zip(labels, out)])
        return out

    def set_plotopts(self, d=None, **kwargs):
        if d != None:
            self.__plotopts = d
        if self.__plotopts == None:
            self.__plotopts = {}
        ...
        for si in s:
            ...
            if isinstance(si, (tuple, list)):
                ...
                if not isinstance(si, str):
                    raise ValueError("inputs are expected to be strings")
                if len(si.strip()) == 0:
                    continue
                ...
                while len(x):
                    for r in b:
                        ...
                        raise RuntimeError(
                            'could not parse channel definition string "%s"' % x
                        )
            if len(row) == 4:
                ind = " ".join(row.pop(0))
                try:
                    ...
                    raise RuntimeError(
                        'failed to interpret string "%s" as an index' % ind
                    )
            ...
            if len(row) != 3:
                raise RuntimeError(
                    'channel definition string "%s" should have 3 or 4 fields' % si
                )
    ...
    def __grok_filtering_opts(self, exclude, keep, filters_as, coords):
        ...
        if exclude == "auto":
            ...
            exclude = numpy.logical_not(
                (x.flatten() ** 2 + y.flatten() ** 2) ** 0.5 <= 1.02
            )
    ...
    def CAR(self, exclude="auto", keep="all", filters_as="columns", coords=None):
        ...
        if self.shape[0] != self.size:
            raise TypeError(
                "CAR() method can only be called on single-column ChannelSet objects"
            )
        exclude, keep, filters_as_rows = self.__grok_filtering_opts(
            exclude, keep, filters_as, coords
        )
        n = len(self)
    ...
    def SLAP(
        ...
        if self.shape[0] != self.size:
            raise TypeError(
                "SLAP() method can only be called on single-column ChannelSet objects"
            )
        exclude, keep, filters_as_rows = self.__grok_filtering_opts(
            exclude, keep, filters_as, coords
        )
        ...
        if len(bad) == 1:
            raise ValueError(
                "channel %s must be excluded, since its %s position is unknown"
                % (",".join(bad), type)
            )
        elif len(bad):
            raise ValueError(
                "the following must be excluded, since their %s positions are unknown: %s"
                % (type, ",".join(bad))
            )
        q[:, exclude] = numpy.inf
        q[exclude, :] = numpy.inf
    ...
    def McFarlandLaplacian(self, type="large", filters_as="columns"):
        ...
        for lab in scheme.split():
            ...
            rows = dict(
                [(k, i) for i, k in enumerate("FP AF F FC C CP P PO O I".split())]
            )
            cols = dict(
                [(k, i) for i, k in enumerate("9 7 5 3 1 0 2 4 6 8 10".split())]
            )
        ...
        for col, c in enumerate(self.flat):
            ...
            for direction, flanker in list(flankers.items()):
                if flanker not in rc2label:
                    continue
                if rc2label[flanker] not in selflabs:
                    continue  # TODO: include this criterion?
                target = schpos[rc2label[flanker]]
                dsq, row, lab = min(
                    [
                        (sqdist(schpos[lab], target), index, lab)
                        for index, lab in enumerate(selflabs)
                    ]
                )
                if lab != thislab:
                    W[row, col] = 1
            ...
            for direction in list(flankers.keys()):
                if direction in surroundlabels:
                    continue
                if -direction not in surroundlabels:
                    continue
        ...
        if 0:
            if type == "large":
                filt = {"C3": "F3  P3  Cz T7", "C4": "F4  P4  Cz T8"}  # old-style
            elif type == "small":
                filt = {
                    "C3": "FC3 CP3 C1 C5",
                    "C4": "FC4 CP4 C2 C6",
                ...
                raise ValueError('type must be "large" or "small"')
            allnames = " ".join(list(filt.keys()) + list(filt.values())).split()
            ind = self.find_labels(
                allnames,
                dict_output=True,
                error_if_not=" (required for %s Laplacian on [%s])"
                % (type, ",".join(list(filt.keys()))),
            )
        ...
        return (
            W,
            masterdict,
        )  # TODO: returning masterdict is an interim solution. the best final solution might be to make the result valid for only one montage, and use a hard-coded masterdict that has been tweaked by hand in place of the small hard-coded dicts in the old method above
    ...
    def spfilt(self, W, inputs=None, exclude="auto", filters_as="columns", coords=None):
        ...
        if self.shape[0] != self.size:
            raise TypeError(
                "spfilt() method can only be called on single-column ChannelSet objects"
            )
        exclude, keep, filters_as_rows = self.__grok_filtering_opts(
            exclude=exclude, keep="all", filters_as=filters_as, coords=coords
        )
        n = len(self)
        ...
        if Win.shape[0] != nInputs:
            raise ValueError("W should have %d %s" % (nInputs, inputDimName))
    ...
    def round(self, precision=1e-5):
        for c in self.flat:
            for k, v in list(c.pos.items()):
                c.pos[k] = precision * round(v / precision)
            for k, v in list(c.neg.items()):
                c.neg[k] = precision * round(v / precision)
            c._update()
...
def trodeplot(
    ...
    if act.shape[0] not in (0, len(channels)):
        raise ValueError("wrong number of activation values")
    if len(act.shape) > 1 and act.shape[1] > 1:
        ...
        if ax == None:
            if not hold:
                pylab.clf()
            ax = SigTools.Plotting.subplots(
                layout, visible=False, xlim=(-1, 1), ylim=(-1, 1)
            )
        h = []
    ...
    if len(act):
        if clim == "auto":
            clim = numpy.r_[min(act), max(act)]
        if balance != None:
            clim = balance + numpy.r_[-1, +1] * max(abs(numpy.asarray(clim) - balance))
    ...
    res = 70
    if isinstance(surf, (int, float)) and surf > 1:
        res = surf
    if isinstance(cmap, str):
        cmap = pylab.cm.get_cmap(cmap)
    if cmap == None:
        cmap = getattr(pylab.cm, "kelvin_i", pylab.cm.jet)

    if contour == "auto":
        contour = surf
    if surf or contour:
        expand = 1.1
        if not isinstance(expand, (tuple, list, numpy.ndarray)):
            expand = [expand]
        positions = [get_position(ch, default=None, coords=coords) for ch in chk]
        chx, chy, chz = list(
            zip(*[p + (act[i],) for i, p in enumerate(positions) if p != None])
        )
        xl = numpy.array([min(chx), max(chx)])
        xl = xl.mean() + expand[0] * (xl - xl.mean())
        yl = numpy.array([min(chy), max(chy)])
        yl = yl.mean() + expand[-1] * (yl - yl.mean())
        xl = yl = [-1.0, 1.0]
        ...
        if surf:
            if use_pcolor:
                ...
                h["surf"] = matplotlib.image.NonUniformImage(
                    ax,
        ...
        if mask_surf or mask_contour:
            if isinstance(mask_surf, float):
                grow = mask_surf
            elif isinstance(mask_contour, float):
                grow = mask_contour
            else:
                grow = 1.1
            ...
            points = hull + (hull[0],)
            codes = [matplotlib.path.Path.MOVETO] + [matplotlib.path.Path.LINETO] * len(
                hull
            )
            ...
            def squares(points):
                ...
                d = (
                    0.5
                    * min([d for d in SigTools.sqdist(points).flat if d != 0]) ** 0.5
                )
                ...
                for vi in v:
                    ...
                    codes = (
                        ...
                        + (matplotlib.path.Path.LINETO,) * len(vi)
                    )
                return points, codes

            if mask_surf == "square" or (not masksurf and mask_contour == "square"):
                points, codes = squares(
                    positions
                )  # TODO: non-convex rectilinear hull, for ECoG grids
            patch = matplotlib.patches.PathPatch(
                matplotlib.path.Path(points, codes), edgecolor="none", facecolor="none"
            )
            ax.add_patch(patch)
            h["mask"] = patch
            if mask_surf:
                if h.get("surf", None):
                    h["surf"].set_clip_path(patch)
            if mask_contour:
                for hh in getattr(h.get("contour"), "collections", []):
                    hh.set_clip_path(patch)

    if highlight_connected:
        for i, r in enumerate(ref):
            if r.lower() not in chk:
                channels.append(r)
                chk.append(r.lower())
            ref[i] = r.lower()
        for i, g in enumerate(gnd):
            if g.lower() not in chk:
                channels.append(g)
                chk.append(g.lower())
            gnd[i] = g.lower()
    if scheme == "auto":
        if len(channels) == 0:
            scheme = "extended1020 eyes_monopolar ears"
        else:
            scheme = not surf
    if scheme:
        if not isinstance(scheme, (tuple, list, str, numpy.ndarray)):
            scheme = None
    ...
    chx, chy, chz = list(
        zip(*[get_position(ch, coords=coords) + (act[i],) for i, ch in enumerate(chk)])
    )
    ...
    if ears:
        ...
        x = (
            numpy.array(
                [0.492, 0.510, 0.518, 0.5299, 0.5419, 0.54, 0.547, 0.532, 0.510, 0.484]
            )
            * 2
        )
        y = (
    ...
    if head or h.get("surf") or h.get("contour"):  # or h.get('nose') or h.get('ears'):
        ...
        ax.add_patch(h["head"])
        if head:
            h["head"].set_edgecolor(color)
        if not mask_surf and h.get("surf"):
            h["surf"].set_clip_path(h["head"])
        if not mask_contour:
            for hh in getattr(h.get("contour"), "collections", []):
                hh.set_clip_path(h["head"])
    ...
    if eeg and eegbg:
        ...
        if eegbg == "auto":
            ...
            gpos = [
                get_position(ch, coords=coords) for ch in " ".join(gridlines).split()
            ]
            eegbg = head and not surf and False not in [gp in drawing for gp in gpos]
        if eegbg:
            circ = pylab.Circle(
                (0.0, 0.0), radius=0.82, edgecolor=color, facecolor="none", zorder=3
            )
            ax.add_patch(circ)
            h["gridlines"] = [circ]
            for gl in gridlines:
                x, y = list(
                    zip(*[get_position(ch, coords=coords) for ch in gl.split()])
                )
                h["gridlines"] += pylab.plot(
                    x, y, linestyle="-", marker="None", color=color, zorder=3
                )
            h["gridlines"] += pylab.plot(
                [0, 0], [-1, +1], linestyle="-", marker="None", color=color, zorder=3
            )
            h["gridlines"] += pylab.plot(
                [-1, +1], [0, 0], linestyle="-", marker="None", color=color, zorder=3
            )
    h["dots"] = []
    h["labels"] = []
    h["indices"] = []
    ...
    dfc = None
    if dfc in ("none", "None", None):
        dfc = "w"
    if facecolor not in ("auto", "connected") and isinstance(
        facecolor, (str, tuple, list, float)
    ):
        dfc = facecolor
    for i, (x, y) in enumerate(zip(chx, chy)):
        if fontsize == "auto":
            fontsize = (
                numpy.diff(ax.get_position(), axis=0).flat[-1]
        ...
        z = act[i]
        if facecolor == "auto":
            actface = not surf or eeg and (x**2 + y**2) ** 0.5 > 1.005
        else:
            actface = isinstance(facecolor, (int, bool)) and facecolor
        if facecolor == "connected":
            actface = False
            if i < number_of_explicitly_given_channels:
                circ.set_facecolor([1.0, 0.5, 0.5])
            elif chk[i] in ref:
                circ.set_facecolor([0.5, 0.5, 1.0])
            elif chk[i] in gnd:
                circ.set_facecolor([0.5, 1.0, 0.5])
        ...
        if labels == "auto":
            labels = {True: (0, 0), False: False}[troderadius >= 0.04]
        if labels:
            xy = numpy.array([x, y])
            if isinstance(labels, (tuple, list, numpy.ndarray)):
                xy = xy + numpy.asarray(labels) * troderadius
            txt = pylab.Text(xy[0], xy[1], text=channels[i], **props)
            ax.add_artist(txt)
            h["labels"].append(txt)
        if indices == "auto":
            indices = {True: (1, 1), False: False}[troderadius >= 0.04]
        if indices:
            xy = numpy.array([x, y])
            if indices == True:
                indices = (1, 1)
            if isinstance(indices, (tuple, list, numpy.ndarray)):
                xy = xy + numpy.asarray(indices) * troderadius
            if i < number_of_explicitly_given_channels:
                text = str(i + int(bool(one_based)))
            elif chk[i] in ref:
                text = "ref"
            elif chk[i] in gnd:
                text = "gnd"
                ...
                text = ""
            txt = pylab.Text(xy[0], xy[1], text=text, **props)
            ax.add_artist(txt)
            h["indices"].append(txt)
...
def match_schemes(channels, force_schemes=None):
    ...
    if force_schemes == None:
        ...
        if isinstance(force_schemes, str):
            force_schemes = force_schemes.split()
        matchingschemes = dict(
            [
                (sch, ElectrodePositions.schemes[sch.lower()].split())
    ...
    for v in list(matchingschemes.values()):
        for vi in v:
            if vi.lower() not in lchannels:
                universe[vi.lower()] = vi
    universe = channels + sorted(universe.values())
    return universe, matchingschemes


def get_position(
    label, type="schematic2D", default=(numpy.nan, numpy.nan), coords=None
):
    if coords == None:
        return getattr(ElectrodePositions, type).get(label.lower(), default)
    if hasattr(coords, type):
        coords = getattr(coords, type)
    elif type in coords:
        coords = coords[type]
