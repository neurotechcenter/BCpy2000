...
def disp(*pargs, **kwargs):
    ...
    inputname = kwargs.pop(
        "inputname", ""
    )  # TODO: is there an equivalent of matlab's inputname call?
    if len(kwargs):
        raise TypeError(
            "%s() got an unexpected keyword argument '%s'"
            % (whoami(), list(kwargs.keys())[0])
        )
    spc = "  " * indent
    ...
    for a in pargs:
        if isinstance(a, list) or isinstance(a, tuple):
            for i in range(len(a)):
                disp(a[i], fmt=fmt, indent=indent, inputname=inputname + "[%d]" % i)
            continue
        if isinstance(a, dict):
            for i in list(a.keys()):
                disp(a[i], fmt=fmt, indent=indent, inputname=inputname + "['%s']" % i)
            continue
        if not isnumpyarray(a):
            print(inputname + " = " + a.__repr__() + "\n")
            continue
        if len(inputname):
            print("%s = " % inputname)
        a = project(a, 1)
        ...
        for i in range(0, nrows):
            ...
            for j in range(0, ncols):
                ...
                if comp:
                    s += aistr[i][j] + " " * int(colw - nchars[i, j].imag)
                if j < ncols - 1:
                    s += ",  "
            print(s)
        print("")
...
def cat(*pargs, **kwargs):
    ...
    if len(pargs) == 1 and isinstance(pargs[0], (tuple, list)):
        pargs = pargs[0]
...
def mad(a, b=None, axis=None):
    ...
    if b != None:
        ...
        if a.size == 1:
            ...
            a = project(a, max(axis, len(b.shape) - 1))
        if b.size == 1:
            b.shape = (1,)
        else:
            b = project(b, max(axis, len(a.shape) - 1))
        if a.size != 1 and b.size != 1 and a.shape != b.shape:
            raise ValueError("unmatched shapes")
        a = a - b
    a = numpy.abs(a)
    if axis != None:
        a = project(a, axis)
...
def isequal(a, b):
    ...
    floats = (
        ...
        numpy.float32,
        numpy.float64,
        getattr(numpy, "float96", numpy.float),
        getattr(numpy, "float128", numpy.float),
    )
    if (
        isinstance(a, floats)
        and isinstance(b, floats)
        and numpy.isnan(a)
        and numpy.isnan(b)
    ):
        return True
...
def edges(x, axis=-1, direction=+1, exceed=0.0, return_logical=False):
    ...
    )  # v. important to recast, in case pre-existing data type is unsigned
    edges = direction * diffx(x, axis=axis) > exceed
    if not return_logical:
        edges = sorted(set(edges.nonzero()[axis]))
    return edges
...
def refrac(indices, interval, fs=1.0):
    ...
    if isnumpyarray(indices):
        ...
        if indices.dtype == numpy.bool:
            if indices.size != max(indices.shape):
                raise ValueError(
                    "if a logical array is supplied, it should only have one non-singleton dimension"
                )
...
def epochs(
    ...
    """  ###
    out = []
    if return_array and return_partial:
        raise ValueError(
            "the return_array and return_partial options cannot both be set to True"
        )
    ...
    if not isnumpyarray(x):
        x = numpy.asarray(x)
    sub = [slice(None, None) for i in x.shape]
    start_indices = refrac(
        start_indices, interval=refractory, fs=fs
    )  # even if refractory=0, this also wrangles the type/shape of start_indices as necessary
    indices_used = []
...
def summarize(a):
    ...
    if isinstance(a, numpy.matrix):
        ...
        atype = "method " + ".".join(
            (a.__self__.__class__.__module__, a.__self__.__class__.__name__, a.__name__)
        )
        obj = getattr(a, "im_self", None)
        if obj != None:
            atype += " of %s instance 0x%08x" % (obj.__class__.__name__, id(obj))
        return atype
    else:
    ...
    try:
        order = a.flags.farray and "order='F'" or "order='C'"
    except:
        order = ""
...
def reportstruct(x, indent=None, maxstrlen=24):
    ...
    if not isinstance(x, dict) and hasattr(x, "__dict__"):
        x = x.__dict__
    if not isinstance(x, dict) or False in [isinstance(f, str) for f in x]:
        return summarize(x)
    if len(x) == 0:
        return s
    ...
    for f in fields:
        ...
        if isinstance(v, float):
            ...
            isinstance(v, (list, tuple, float, int, bool, str, type(None)))
            and len(repr(v)) < maxstrlen
            and "\n" not in repr(v)
        ):
            s += repr(v) + "\n"
        elif isinstance(
            ...
            if hasattr(v, "_fields") or (
                isinstance(v, dict) and False not in [isinstance(ff, str) for ff in v]
            ):
                vcl = v.__class__.__name__
                if vcl != xcl and vcl != "sstruct":
                    s += "(%s)" % (vcl)
                s += "\n" + reportstruct(
                    v, indent=indent + ndashes + 1, maxstrlen=maxstrlen
                )
            else:
                s += summarize(v) + "\n"
...
class ghostsstruct(object):
    ...
    def __repr__(self):
        p = self._parent
        return "non-existent .%s field of <%s.%s instance at 0x%08X>" % (
            self._fieldname,
...
class sstruct(object):
    ...
    def __getattr__(self, f):
        ...
        if allowed == None:
            ...
            allowed = f in allowed
        if not allowed or f.startswith("_"):
            raise AttributeError(
                "'%s' object has no attribute '%s'" % (self.__class__.__name__, f)
            )
    ...
    def __setattr__(self, f, v):
        ...
        if "." in f:
            self._setfield(f, v)
            return
        if not f.startswith("_") and f not in self._fields:
            self._fields.append(f)
        self.__dict__[f] = v

    def __delattr__(self, f):
        return self._rmfield(f)
    ...
    def _reverse(self):
        ...
        for k, v in list(self.__dict__.items()):
            if not k.startswith("_") and hasattr(v, "_reverse"):
                v._reverse()
    ...
    def _inherit(self, _baseobj=None, _recursive=False, **kwargs):
        ...
        if _baseobj != None:
            if not isinstance(_baseobj, sstruct):
                _baseobj = sstruct(_baseobj)
            if _recursive:
                for f, v in _baseobj._allitems():
                    self._setfield(f, v)
            else:
                for f in _baseobj._fields:
                    setattr(self, f, getattr(_baseobj, f))
    ...
    def copy(self, deep=False):
        ...
        for f, v in self._allitems():
            ...
            c._setfield(f, v)
        [
            c.__dict__.__setitem__(f, copy.deepcopy(v))
    ...
    def __eq__(self, other):
        ...
        sf = self._allfields()
        of = other._allfields()
        if not isequal(sorted(sf), sorted(of)):
            return False
        for f in sf:
            if not isequal(self._getfield(f), other._getfield(f)):
                return False
        return True
    ...
    def __getitem__(self, i):
        if isinstance(i, int):
            ...
            return self._getfield(i)

    def __setitem__(self, i, val):
        return self._setfield(i, val)

    def __delitem__(self, i):
        return self._rmfield(i)
    ...
    def _allfields(self):
        ...
        for f in self._fields:
            v = self.__dict__[f]
            if isinstance(v, sstruct):
                result += [f + "." + vf for vf in v._allfields()]
            else:
                result.append(f)
        return tuple(result)
    ...
    def _setfield(self, field=None, val=None, **kwargs):
        ...
        if field != None:
            ...
            if len(field) == 1:
                setattr(self, field[0], val)
                return self
            if field[0] not in self._fields:
                setattr(self, field[0], sstruct())
            sub = self.__dict__[field[0]]
            if not isinstance(sub, sstruct):
                raise AttributeError("field '%s' cannot accept subfields" % field[0])
            sub._setfield(field[1:], val)
        for field, val in list(kwargs.items()):
            self._setfield(field, val)
        return self
    ...
    def _getfield(self, field, *pargs):
        ...
        if len(pargs) > 1:
            raise TypeError(
                "_getfield() takes at most 3 arguments (%d given)" % len(pargs) + 2
            )
        if isinstance(field, str):
            field = field.strip(".").split(".")
        if not isinstance(field, (tuple, list)):
    ...
    def _rmfield(self, field):
        ...
        if len(parts) == 1:
            ...
            if not allowed:
                raise AttributeError('the "%s" field cannot be removed' % field)
...
def ssdiff(obj, *objs):
    ...
    for field in fields:
        for obj, allfields_thisobj in zip(objs[1:], allfields_eachobj[1:]):
            if (
                field not in allfields_thisobj
                or field not in allfields_eachobj[0]
                or obj[field] != objs[0][field]
            ):
                if return_common:
                    rm.append(field)
                else:
...
def loadmat(filename, **kwargs):
    ...
    if "struct_as_record" not in kwargs:
        kwargs["struct_as_record"] = False
    if "squeeze_me" not in kwargs:
        kwargs["squeeze_me"] = False
...
def savemat(filename, d, append=False, oned_as="row"):
    ...
    before = set(d.keys())
    selectval = lambda x: isinstance(x, (str, int, float, numpy.ndarray)) or (
        isinstance(x, (tuple, list))
        and not False in [isinstance(y, (float, int)) for y in x]
    )
    ...
    for k in gone:
        print("warning: unable to save item  %s: %s" % (repr(k), summarize(d[k])))
    if len(dd) == 0:
...
def pickle(obj, targetfile, protocol=2, append=False):
    ...
    if isinstance(targetfile, str):
        if targetfile.lower().endswith(".gz"):
            openfn = gzip.open
        else:
            openfn = open
