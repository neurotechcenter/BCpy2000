...
def flatten(s, include=None, exclude=None, labels=False):
    if isinstance(s, sstruct):
        kvtuples = s._allitems()
    elif isinstance(s, dict):
        kvtuples = sorted(s.items())
    else:
        if labels:
            if isinstance(s, (tuple, list, numpy.ndarray)):
                s = numpy.asarray(s)
                return [
                    str(list(numpy.unravel_index(i, s.shape))).replace(" ", "")
    ...
    vals = []
    for k, v in kvtuples:
        if k != None and include != None and k not in include:
            continue
        if k != None and exclude != None and k in exclude:
            continue
        v = flatten(v, labels=labels)
        if labels:
            vals += ["." + k + vi for vi in v]
        else:
            vals.append(v)
    if labels:
        return vals
        ...
        return numpy.concatenate(vals, axis=0)
...
def unflatten(s, vals, include=None, exclude=None):
    if isinstance(s, sstruct):
        ...
        kvtuples = list(zip([None for si in s], s))
    pos = 0
    for i, (k, v) in enumerate(kvtuples):
        ...
        if isinstance(v, numpy.ndarray):
            n = v.size
            v.flat = vals[pos : pos + n]
        elif isinstance(v, list):
            n = len(v)
            v[:] = vals[pos : pos + n]
        elif isinstance(v, tuple):
            n = len(v)
            v = v.__class__(vals[pos : pos + n])
        else:
            n = 1
            v = v.__class__(vals[pos])
        pos += n
        if isinstance(s, sstruct):
            s._setfield(k, v)
        elif isinstance(s, dict):
...
class NoSubclassMethod(Exception):
    pass
...
class objective(sstruct):
    ...
    def fix(self, *pargs, **kwargs):
        ...
        for args in list(pargs) + list(kwargs.keys()):
            if not isinstance(args, (list, tuple)):
                args = (args,)
            for arg in args:
                if arg in self._free:
                    self._free.remove(arg)
        for k, v in list(kwargs.items()):
            self._setfield(k, v)
    ...
    def _f(self, x, force1D=False, *unused):
        ...
        f = self.eval()
        if force1D or not isinstance(f, float):
            f = flatten(f)
        return f
    ...
    def _df(self, x, force2D=False, *unused):
        ...
        if df == None:
            if not self._scalar:
                raise RuntimeError(
                    "if using the _deriv structure, zero-order function output should be scalar"
                )
            ...
            if self._scalar:
                if numpy.asarray(df).shape != (self._np,):
                    raise RuntimeError(
                        "if deriv() returns an output and zero-order output is scalar, deriv() output shape should (nparams,)"
                    )
            else:
                if df.shape != (self._nf, self._np):
                    raise RuntimeError(
                        "if deriv() returns an output and zero-order output is non-scalar, deriv() output shape should (noutputs, nparams)"
                    )
        if force2D and len(numpy.asarray(df).shape) == 1:
            df = numpy.expand_dims(df, 0)
        return df

    def _ddf(self, x, force2D=False, *unused):
        unflatten(self, x, include=self._free)
        if self._dderiv != None:
            unflatten(self._dderiv, x * 0.0, include=self._free)
        ddf = self.dderiv()
        if ddf == None:
            if not self._scalar:
                raise RuntimeError(
                    "if using the _dderiv structure, zero-order function output should be scalar"
                )
            ...
            if self._scalar:
                if numpy.asarray(ddf).shape != (self._np,):
                    raise RuntimeError(
                        "if dderiv() returns an output and zero-order output is scalar, dderiv() output shape should (nparams,)"
                    )
            else:
                if ddf.shape != (self._nf, self._np):
                    raise RuntimeError(
                        "if dderiv() returns an output and zero-order output is non-scalar, dderiv() output shape should (noutputs, nparams)"
                    )
        if force2D and len(numpy.asarray(ddf).shape) == 1:
            ddf = numpy.expand_dims(ddf, 0)
        return ddf
    ...
    def eval(self):
        ...
        """  ###
        raise NoSubclassMethod(
            "this %s subclass should at least implement an eval() method"
    ...
    def deriv(self):
        ...
        """  ###
        raise NoSubclassMethod(
            "this %s subclass has not implemented a first-order deriv() method"
            % self.__class__.__name__
        )
    ...
    def dderiv(self):
        ...
        """  ###
        raise NoSubclassMethod(
            "this %s subclass has not implemented a second-order dderiv() method"
            % self.__class__.__name__
        )
    ...
    def hessian(self):
        ...
        """  ###   # TODO(HHH)
        raise NoSubclassMethod(
            "this %s subclass has not implemented a hessian() method"
            % self.__class__.__name__
        )
    ...
    def _reset(self):
        ...
        try:
            ...
            for k, v in self._allitems():
                if k in self._free:
                    self._dderiv._setfield(k, v * 0.0)
    ...
    def optimize(self, *pargs):
        ...
        p = scipy.optimize.fmin_bfgs(
            f=self._f, x0=flat_p0_free, fprime=self._df, disp=False
        )
    ...
    def chgrad(self, plot=False, **kwargs):
        ...
        out.flabels = [
            "f" + x for x in flatten(self.eval(), include=self._free, labels=True)
        ]
        ...
        if plot:
            ...
            for i, k in enumerate(fields):
                ...
                pylab.cla()
                ax = pylab.gca()
                (ax.indicator,) = pylab.plot(
                    [numpy.nan, numpy.nan],
                    [numpy.nan, numpy.nan],
                    color=(1, 0, 1),
                    linewidth=2,
                )
                v.handles = pylab.plot(
...
class chgradtest(objective):
    ...
    def __init__(self, p, x):
        ...
        if self.p.size != 2:
            raise ValueError("p should be a 2-element array")
    ...
    def eval(self):
        ...
        pdim = ([0] + [i for i, d in enumerate(x.shape) if d != 1])[
            -1
        ]  # last non-singular dim, or 0 if none
    ...
    def hessian(self):
        if self.x.size != 1:
            raise NoSubclassMethod()  # pretend we're not here
        x, u, z = self.x, self._u, self._z
        p, q = numpy.asarray(self.p, dtype=numpy.float64).flat
...
class psifit(objective):
    def __init__(self, x, y, **kwargs):
        ...
        self.link = logistic
        self.fix()
        self.free(shift=0.0, logscale=0.0)
        # self.shift = 0.0
        # self.logscale = 0.0
        # self._free = set(['shift', 'logscale'])
        self._update(kwargs)
        af = self._allfields()
        if "loglower" in af and self.loglower > -numpy.inf:
            self._free.add("loglower")
        else:
            self.loglower = -numpy.inf
        if "logupper" in af and self.logupper > -numpy.inf:
            self._free.add("logupper")
        else:
            self.logupper = -numpy.inf
        if "logyoked" in af and self.logyoked > -numpy.inf:
            self._free.add("logyoked")
        else:
            self.logyoked = -numpy.inf
        if "logyoked" in self._free and "loglower" in self._free:
            self._free.remove("loglower")
        if "logyoked" in self._free and "logupper" in self._free:
            self._free.remove("logupper")
    ...
    def intermediates(self, x=None, y=None):
        ...
        h = 1.0 - el - eu - 2.0 * eq
        if h < 0.0:
            raise RuntimeError("impossible parameter values")
        u = eb * (x - a)
        f = self.link(u)
