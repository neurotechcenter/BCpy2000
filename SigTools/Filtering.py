...
def applyfilter(x, b=1.0, a=1.0, axis=-1, zi=None):
    ...
    zf = numpy.array(zi)
    colon = slice(None)
    for sub in numpy.ndindex(
        tuple(shape)
    ):  # have to do it this way because filtering of non-vector signals is still bugged in scipy (non-deterministic output, intermittent crashes)
        sub = list(sub)
        sub[axis] = colon
        v, zf[sub].flat[:] = lfilter(b=b, a=a, x=x[sub].flatten(), zi=zi[sub].flatten())
        y[sub].flat[:] = v.real
    if matrixformat:
        y = numpy.matrix(y, copy=False)
    return y, zf
...
def firdesign(N, Wn, btype="lowpass", width=None, window="hamming", output="ba"):
    ...
    if btype != "lowpass":
        raise RuntimeError("only low-pass FIR filter design implemented so far")  # TODO
    a = [1.0]
...
class causalfilter(object):
    def __init__(
        ...
        order=10,
        type="bandpass",
        method=scipy.signal.filter_design.butter,
        **kwargs
    ):
        """
        ...
        if not isinstance(freq_hz, list) and not isinstance(freq_hz, tuple):
            freq_hz = [freq_hz]
        ...
        self.b, self.a = method(
            N=self.order / 2.0, Wn=tuple(lims), btype=self.type, output="ba", **kwargs
        )
        self.state = None
        self.samples_filtered = 0

    def __repr__(self):
        basestr = "<%s.%s instance at 0x%08X>" % (
    ...
    def filter(self, x, axis=None, reset=False, filtfilt=False):
        ...
        if filtfilt:
            self.reset()
            y, self.state = applyfilter(
                x=flip(y, axis), b=self.b, a=self.a, axis=axis, zi=self.state
            )
            y = flip(y, axis)
...
def fftfilt(d, fs, band=(58, 62), mode="bandstop", axis=0, dB=20):
    ...
    if not isinstance(mode, (tuple, list)):
        mode = [mode]
    if not isinstance(band[0], (tuple, list)):
        band = [band]
    if not isinstance(dB, (tuple, list)):
        dB = [dB]
    if len(mode) == 1:
        mode = mode * len(band)
    if len(dB) == 1:
    ...
    for mode, band, dB in zip(mode, band, dB):
        ...
        if mode == "highpass":
            w = w * (depth + (1.0 - depth) * shoulder(f, band + [fs, fs]))
        elif mode == "lowpass":
            w = w * (depth + (1.0 - depth) * shoulder(f, [-fs, -fs] + band))
        elif mode == "bandpass":
            w = w * (depth + (1.0 - depth) * shoulder(f, band))
        elif mode == "bandstop":
            w = w * (1.0 - (1.0 - depth) * shoulder(f, band))
        else:
            raise ValueError('unrecognized filtering mode "%s"' % mode)
    d = numpy.real(ifft(D * w, axis=axis))
    return d


def filt(
    d, fs, band=(58, 62), mode="bandstop", axis=0, order=3, reverse=True, filtfilt=True
):
    """
...
class fader(object):
    ...
    def __iadd__(self, g):
        self.queue = numpy.concatenate(
            (self.queue, numpy.array(g, ndmin=2) * numpy.ones(self.gain.shape)), axis=1
        )
        return self
    ...
    def apply(self, x):
        ...
        if extra > 0:
            g = numpy.concatenate((g, self.gain.repeat(extra, axis=1)), axis=1)
