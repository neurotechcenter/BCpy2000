...
if kernel32dll != None:
	...
	def prectime():
		...
		kernel32dll.QueryPerformanceFrequency(ctypes.c_void_p(ctypes.addressof(timebase)))
	...
	def SetProcessPriority(p):
		...
		if not p in list(process_priorities.values()): raise ProcessAPIError('unrecognized priority value')
	...
	def SetProcessAffinity(aff):
		if isinstance(aff, (tuple,list)): aff = sum([2**x for x in aff])
	...
	def SetThreadPriority(p):
		...
		if not p in list(thread_priorities.values()): raise ThreadAPIError('unrecognized priority value')
