...
def IfStringThenRawString( x ):    return x.encode( 'utf-8' ) if isinstance( x, str ) else x
...
try:
	...
	exec('import os,sys', __IPYTHON__.user_ns) # in case this is being run via import rather than execute
	__OLDER_IPYTHON__ = False
else:
	exec('import os,sys', __IPYTHON__.shell.user_ns) # in case this is being run via import rather than execute
...
class MyMagics(Magics):
	...
	dbstop = None
	try:
		if not dbstop: from IPython.Debugger import Tracer; dbstop = Tracer() # IPython v 0.10 and earlier
	except: pass
	try:
		if not dbstop: from IPython.core.debugger import Tracer; dbstop = Tracer() # IPython v 0.11 and later
	except: pass
	try:
		if not dbstop: from IPython.core import debugger; dbstop = debugger.Pdb().set_trace  # Some other f***ing version of IWishYoudStopMovingTheGoalposts
	except: pass
		...
		for root, dirnames, filenames in os.walk( root_dir ):
			...
			for filename in filenames:
				...
				if '--filename' in opts:
					...
					continue
				matches.append( file_path )
				try: file_handle = open( file_path )
				except IOError:
				...
				for line_number, line in enumerate( file_handle ):
					...
					if hit:
						try: sys.stdout.write( '%s:%d:\n    %s\n\n' % ( file_path.replace( '\\', '/' ), line_number + 1, str( line ).strip()[ : limit ] ) )
						except UnicodeDecodeError: sys.stdout.write( '%s:%d  matches (no preview available)\n\n' % ( file_path, line_number + 1 ) )
						sys.stdout.flush()
		if len( matches ) == 0:
...
"""###
		...
		try: import matplotlib
			...
			try: exec('import matplotlib, pylab', ipython)
...
"""
		...
		startup_file = os.path.join( startup_dir, 'BCPy2000Shortcuts.ipy' )  # cannot be a .py file, must be an .ipy - the latter seem to be executed later, when ipython is running and its instance can be accessed with get_ipython()
