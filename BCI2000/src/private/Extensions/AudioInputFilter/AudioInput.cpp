...
MMRESULT AudioInput::EnumDevices()
{
	MMRESULT result;
...
void CALLBACK AudioInput::RecordingCallback(HWAVEIN hwi, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
	////wprintf("CALLBACK [%x,%i,%i] - ",dwInstance,dwParam1,dwParam2);
	AudioInput *mr = (AudioInput *)dwInstance;
	switch(uMsg) {
	case MM_WIM_OPEN:
		////wprintf("Open\n");
		break;
...
MMRESULT AudioInput::SetRecordingFormat(unsigned int pNumChannels, unsigned char pNumBitsPerChannel, unsigned int pSamplingRate)
{
...
MMRESULT AudioInput::SetDevice(unsigned int pDeviceID)
{
...
MMRESULT AudioInput::SetDevice(std::string pDevicePartialName)
{
...
MMRESULT AudioInput::OpenSelectedDevice()
{
...
MMRESULT AudioInput::OpenFileForRecording()
{
	MMRESULT result;
...
MMRESULT AudioInput::InitBuffer(unsigned int pMilliseconds, unsigned int pBufferSize)
{
	...
	for( unsigned int ch = 0; ch < mRecordingFormat.nChannels; ch++ )
		mSignal.push_back( new RingBuffer( (mRecordingFormat.nSamplesPerSec * pMilliseconds) / 1000 ) );

	return MMSYSERR_NOERROR;
}

void AudioInput::AddAllBuffers()
{
...
void AudioInput::FreeBuffer()
{
	...
	while( mSignal.size() )
	{
		RingBuffer *b = mSignal.back();
		mSignal.pop_back();
		delete b;
	}
}

MMRESULT AudioInput::CloseDevice()
{
	MMRESULT result;
	FreeBuffer();
	if(mDevice != NULL)
...
void AudioInput::ProcessBuffer()
{
	MMRESULT result;

	unsigned int samplesHere = mWaveHeaderRingBufferPtr->dwBytesRecorded / mRecordingFormat.nBlockAlign;
	mSamplesRecorded += samplesHere;

	char* pBlock = (char*)mWaveHeaderRingBufferPtr->lpData;
	for( unsigned int iBlock = 0; iBlock < samplesHere; iBlock++, pBlock += mRecordingFormat.nBlockAlign )
	{
		for( unsigned int iChannel = 0; iChannel < mRecordingFormat.nChannels; iChannel++ )
		{
			double val;
			switch( mRecordingFormat.wBitsPerSample )
			{
				case  8: val = (double)(((char*)pBlock)[iChannel]) / 128.0;
				case 16: val = (double)(((short*)pBlock)[iChannel]) / 32768.0;
				case 24: val = (double)(((long*)pBlock)[iChannel]) / 8388608.0;
				case 32: val = (double)(((long*)pBlock)[iChannel]) / 2147483648.0;
			}
			mSignal[iChannel]->Write( val );
		}
	}
	...
	if( mDestFile )
	{
		...
		if(nWritten != mWaveHeaderRingBufferPtr->dwBytesRecorded) bciwarn << "ERR: Didn't record what was asked";
	}
...
bool
AudioInput::Read( unsigned int channel, double& x )
{
	if( channel >= mSignal.size() ) bcierr << "illegal audio channel buffer " << channel << " requested" << endl;
	return mSignal[channel]->Read( x );
}
