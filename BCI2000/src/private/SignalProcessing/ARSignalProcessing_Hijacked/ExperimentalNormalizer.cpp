////////////////////////////////////////////////////////////////////////////////
// $Id: Normalizer.cpp 3798 2012-02-01 18:07:06Z mellinger $
...
RegisterFilter( ExperimentalNormalizer, 2.E1 );

#define Normalizer ExperimentalNormalizer
...
void
Normalizer::WriteAdaptedParameters()
{
  if( mOutputPrmFile.size() )
  {
    ofstream prmFile;
    ...
    if( prmFile.is_open() )
    {
      ...
      prmFile << p->Section() << " " << p->Type() << " " << p->Name() << "= " << mOffsets.size() << "      ";
	  for( std::vector<double>::iterator i = mOffsets.begin(); i != mOffsets.end(); i++ ) prmFile << *i << "  ";
      prmFile << " // written " << timeBuffer << endl;
      ParamRef q = Parameter( "NormalizerGains" );
      prmFile << q->Section() << " " << q->Type() << " " << q->Name() << "= " << mGains.size() << "      ";
	  for( std::vector<double>::iterator i = mGains.begin(); i != mGains.end(); i++ ) prmFile << *i << "  ";
      prmFile << " // written " << timeBuffer << endl;
      prmFile.close();
    }
    else
