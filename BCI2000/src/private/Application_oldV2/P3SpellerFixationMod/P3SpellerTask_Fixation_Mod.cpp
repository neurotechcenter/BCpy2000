////////////////////////////////////////////////////////////////////////////////
// $Id: P3SpellerTask.cpp 2151 2008-10-02 16:35:43Z mellinger $
...
#include "P3SpellerTask_Fixation_Mod.h"
  ...
  // <-- Fixation End
{
 ...
 // Fixation
 BEGIN_PARAMETER_DEFINITIONS
   "Application:Fixation float BlinkTime= 100ms 100ms 0 % "
     " // Maximum alloted time for a blink ",

   "Application:Fixation float AllowedRadius= 20 20 0 100 "
     " // Allowed radius in % of screen height ",

   "Application:Fixation string FixationCross= images\\fixation_cross.bmp"
     " // Stimulus to fixate on placed in the middle (inputfile)",

   "Application:Fixation string FixationCrossOutside= images\\fixation_cross_outside.bmp"
      " // Fixation changes to this when outside radius (inputfile)",
 END_PARAMETER_DEFINITIONS
 // <-- Fixation End
...
P3SpellerTask::~P3SpellerTask()
{
  ...
  delete mpTextWindow;
  // Fixation Start -->
  // Delete the current Fixation Cross if it exists
  if( mpFixationCross )
  {
    mpFixationCross->Conceal();
    delete mpFixationCross;
    mpFixationCross = NULL;
  }

  // Same with Fixation Char
  if( mpFixationChar )
  {
    mpFixationChar->Conceal();
    delete mpFixationChar;
    mpFixationChar = NULL;
  }
  // <-- Fixation End
}
...
P3SpellerTask::OnPreflight( const SignalProperties& /*Input*/ ) const
{
  ...
  if( Parameter( "LogEyetracker" ) == 1 )
  {
    ...
    bcierr << "This modification of P3Speller is meant to be used with an eyetracker." << endl
           << "Please enable --LogEyetracker=1 on the source application." << endl;

  //Get fixation parameters
  int pBlinkTime = MeasurementUnits::ReadAsTime( Parameter( "BlinkTime" ) );
  float pAllowedRadius = Parameter( "AllowedRadius" ) / 100.0f;

  string pFixationCrossFilename = Parameter( "FixationCross" );
  if( pFixationCrossFilename == "" )
    bcierr << "This experiment requires a central FixationCross." << endl;

  string pFixationCrossOutsideFilename = Parameter( "FixationCrossOutside" );
  if( pFixationCrossOutsideFilename == "" )
    bcierr << "This experiment requires another fixation cross (FixationCrossOutside)." << endl;

  ImageStimulus* pImageStimulus = new ImageStimulus( preflightDisplay );
  if( pImageStimulus != NULL )
    pImageStimulus->SetFile( Parameter( "FixationCross" ) );

  if( pImageStimulus != NULL )
    pImageStimulus->SetFile( Parameter( "FixationCrossOutside" ) );
...
P3SpellerTask::OnInitialize( const SignalProperties& /*Input*/ )
{
  ...
  // Fixation Begin -->
  // Get Fixation Parameters
  mBlinkTime = MeasurementUnits::ReadAsTime( Parameter( "BlinkTime" ) );
  ...
  //Define a rectangle for the fixation cross
  GUI::Rect fixationRect =
  {
    0.5,
    ( ( 1.0f - ( Parameter( "StatusBarSize" ) / 100.0f ) ) / 2.0f )
      + ( Parameter( "StatusBarSize" ) / 100.0f ) - 0.05f,
    0.5,
    ( ( 1.0f - ( Parameter( "StatusBarSize" ) / 100.0f ) ) / 2.0f )
      + ( Parameter( "StatusBarSize" ) / 100.0f ) + 0.05f,
  };
  ...
  mpFixationCross = new ImageStimulus( Display() );
  mpFixationCross->SetFile( Parameter( "FixationCross" ) );
  mpFixationCross->SetRenderingMode( GUI::RenderingMode::Opaque );
  mpFixationCross->SetAspectRatioMode( GUI::AspectRatioModes::AdjustWidth );
  mpFixationCross->SetDisplayRect( fixationRect );
  mpFixationCross->SetPresentationMode( VisualStimulus::ShowHide );
  mpFixationCross->Present();

  // Prepare the fixation char
  mFixationText = "";
  mpFixationChar = new TextStimulus( Display() );
  mpFixationChar->SetText( mFixationText );
  mpFixationChar->SetAspectRatioMode( GUI::AspectRatioModes::AdjustWidth );
  mpFixationChar->SetDisplayRect( fixationRect );
  mpFixationChar->SetPresentationMode( VisualStimulus::ShowHide );
...
// Fixation Start -->
void
P3SpellerTask::Process( const GenericSignal &input, GenericSignal &output )
{
  ...
  float pUserX = ( float )( ( mScreenWidth * pEyetrackerX ) - ( Parameter( "WindowLeft" ) - mScreenVirtualLeft ) )
                 / ( float )Parameter( "WindowWidth" );
  float pUserY = ( float )( ( mScreenHeight * pEyetrackerY ) - ( Parameter( "WindowTop" ) - mScreenVirtualTop ) )
                 / ( float )Parameter( "WindowHeight" );

  // Find Distance From Fixation
  float ylocation = ( ( 1.0f - ( Parameter( "StatusBarSize" ) / 100.0f ) ) / 2.0f ) + ( Parameter( "StatusBarSize" ) / 100.0f );
  ...
  else
  {
    ...
    {
      mBlocksInvalid++;
      if( mBlocksInvalid == mBlinkTime ) //( mInvalidEyes == false )
      {
...
void
P3SpellerTask::ClearTextHistory()
{
...
void
P3SpellerTask::LoadMenu( int                inMenuIdx,
                         GUI::Rect&         ioRect,
...
bool
P3SpellerTask::MultipleMenus() const
{
...
int
P3SpellerTask::MenuRows( int inMenuIdx ) const
{
...
int
P3SpellerTask::MenuCols( int inMenuIdx ) const
{
...
ParamRef
P3SpellerTask::MenuParam( const std::string& inParamName, int inMenuIdx ) const
{
...
string
P3SpellerTask::SummaryFileExtension()
{
  return string( "_" ) + TimeStamp() + "_summary.txt";
}

string
P3SpellerTask::DirectoryFileName() const
{
