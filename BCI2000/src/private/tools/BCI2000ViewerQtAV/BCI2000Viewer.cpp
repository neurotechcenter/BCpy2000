////////////////////////////////////////////////////////////////////////////////
// $Id: BCI2000Viewer.cpp 4607 2013-10-14 13:18:08Z mellinger $
...

{
  ...
  {
    &BCI2000Viewer::ToPlayForward,
    &BCI2000Viewer::GoPlayForward_Enabled,
    NULL,
    "playFwdBtn",
    NULL
  },
...
//  mClock.Start();
  ...
  mpPlayerWindow = new PlayerWindow();
  mpPlayerWindow->show();
  mpPlayerWindow->resize(800, 600);
  //mpPlayerWindow->moveToThread(mpThread);
  connect(mpPlayerWindow, SIGNAL(updateSignalViewerSignal()), this, SLOT(updateSignalViewerSlot()));
  //mpThread->start();
  ...
  mpThread = new UpdateThread(this);
...
{
  mpThread->alive = false;
  mpPlayerWindow->close();
  //mpThread->wait();
  //delete mpThread;
...
void BCI2000Viewer::ToPlayForward()
{ WarpSamplePos( mFile.SignalProperties().Elements() ); }
...
bool BCI2000Viewer::GoPlayForward_Enabled() const
{ return mSamplePos < long( mFile.NumSamples() ) - ui->signalDisplay->Display().NumSamples(); }
...
{
  mFile.Open( inName.toLocal8Bit().constData(), 200*1024*1024 );
  if( !mFile.IsOpen() )
  {
    if( !inName.isEmpty() )
    {
...
void
BCI2000Viewer::WarpSamplePos( long inPosDiff )
{
  if( mSamplePos + inPosDiff < 0 )
  {
    ToFirstSample();
...
void 
BCI2000Viewer::updateSignalViewerSlot()
{
	//mClock.Wait();
	//mpThread->wait();
	//if (mpThread->isFinished()) mpThread->start();
	//else mpThread->wait();
	//mpThread->start();
	//mpThread->wait();
	//QEventLoop loop;
