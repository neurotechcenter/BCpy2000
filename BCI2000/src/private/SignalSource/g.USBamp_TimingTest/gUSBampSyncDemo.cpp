...
const long NUM_SECONDS_RUNNING = 120;	//the number of seconds that the application should acquire data (after this time elapsed the application will be stopped)
const int QUEUE_SIZE = 4;				//the number of GT_GetData calls that will be queued during acquisition to avoid loss of data

//device configuration settings
LPSTR _masterSerial = "UA-2007.10.37";											//specify the serial number of the device used as master
LPSTR _slaveSerials[] = { "UB-2010.03.43", "UB-2010.03.44", "UB-2010.03.47" };	//specify the serial numbers of the devices used as slaves
const int SLAVE_SERIALS_SIZE = 0;												//the number of slave serials specified in slaveSerials
...
const UCHAR NUMBER_OF_CHANNELS = 8;											//the number of channels per device that should be acquired (must equal the size of the _channelsToAcquire array)
...
// code added, jm
					...
					static ofstream timingFile( "timingData.txt" ); // ms timestamps
					timingFile << static_cast<int>( count.QuadPart * 1000 / freq.QuadPart ) << '\n';
// end code addition, jm
