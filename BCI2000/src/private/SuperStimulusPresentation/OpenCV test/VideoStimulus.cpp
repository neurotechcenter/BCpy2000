...
{
    CvCapture* capture = cvCreateFileCapture("C:/BCI2000/prog/videos/1.mp4");

    IplImage* frame = NULL;

    if(!capture)
    {
      bcierr << "Video not opened!";
      return;
    }
    ...
    bciout << "Video Size = " << width << " x " << height;
    bciout << "FPS = " << fps << "; frame coutn is " << frame_count;
