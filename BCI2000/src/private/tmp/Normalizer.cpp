////////////////////////////////////////////////////////////////////////////////
// $Id: Normalizer.cpp 3250 2011-05-05 13:38:39Z mellinger $
  ...
  mDoAdapt( false )
{
 BEGIN_PARAMETER_DEFINITIONS
  ...
  "Filtering intlist Adaptation= 2 2 2 "
    "0 0 3 // 0: no adaptation, "
             "1: zero mean, "
             "2: zero mean, unit variance, "
             "3: normalize around fixed offset value, "
             "(enumeration)",
...
Normalizer::Update()
{
  ...
  {
    bool adaptOffset = ( mAdaptation[ channel ] == zeroMeanUnitVariance || mAdaptation[ channel] == zeroMean );
    bool adaptGain   = ( mAdaptation[ channel ] == zeroMeanUnitVariance || mAdaptation[ channel] == nonCentralNormalization );
    
    if( !adaptOffset && !adaptGain ) continue; // to next channel
    int numValues = 0;
    double meanOfMeans, meanOfVars;
    vector<double> bufferMeans;
    vector<double> bufferVars;
    ...
    for( size_t i = 0; i < mDataBuffers[ channel ].size(); ++i )
    {
      ...
      if( buffer.Fill() == 0 ) continue; // to next buffer
    ...
    if( adaptOffset && !bufferMeans.empty())
    { // Use the mean of means to equalize contributions of all buffers.
      meanOfMeans = accumulate( bufferMeans.begin(), bufferMeans.end(), 0.0 ) / bufferMeans.size();

      mOffsets[ channel ] = static_cast<float>( meanOfMeans );
    ...
    if( !adaptGain ) continue; // to next channel

    double centralValue = mOffsets[ channel ]; // will be equal to meanOfMeans unless using nonCentralNormalization

    for( size_t i = 0; i < mDataBuffers[ channel ].size(); ++i )
    {
      const RingBuffer& buffer = mDataBuffers[ channel ][ i ];
      const RingBuffer::DataVector& data = buffer.Data();
      double bufferDevSqSum = 0.0;

      if( buffer.Fill() == 0 ) continue; // to next buffer
      
      for( size_t j = 0; j < buffer.Fill(); ++j )
      {
        double deviation = data[ j ] - centralValue;
        bufferDevSqSum += deviation * deviation;
      }

      bufferVars.push_back(bufferDevSqSum / buffer.Fill());
    } // buffer loop

    if( adaptGain && !bufferVars.empty())
    { // Again, use the mean of mean(squared deviation) to equalize contributions of all buffers.
      meanOfVars = accumulate( bufferVars.begin(), bufferVars.end(), 0.0 ) / bufferVars.size();
      const double eps = 1e-10;
      if( meanOfVars > eps )
      {
        mGains[ channel ] = 1.0f / ::sqrt(meanOfVars);
