...
class wav:
	...
	def __setattr__(self,key,val):
		if not hasattr(self,key):
			raise AttributeError('cannot add attributes to the wav instances')
		if key == 'y':
			if not isnumpyarray(val):
				raise TypeError('attribute "y" must be a numpy.array')
			if len(val.shape)==1:
				val = val.view()
				val.shape = (val.shape[0],1)
		elif key == 'fs':
			if not isinstance(val,int) or val <=0:
				raise TypeError('attribute \'fs\' must be a positive integer')
		elif key == 'bits':
			...
			if len(val) != 2 or not isinstance(val,tuple) \
			                 ...
			                 or not isinstance(val[1],str):
				raise TypeError('the \'comptype\' attribute should be a tuple containing two strings')
		else:
			raise AttributeError('the \'' + key + '\' attribute is read-only')
	...
	def set_bitdepth(self, bits):
		if bits == 8:
			self.__dict__['nbytes'] = 1
			self.__dict__['dtype'] = '<u1'
		elif bits == 16:
			self.__dict__['nbytes'] = 2
			self.__dict__['dtype'] = '<i2'
		elif bits == 24:
			self.__dict__['nbytes'] = 4
			self.__dict__['dtype'] = '<i4'
		elif bits == 32:
			self.__dict__['nbytes'] = 4
			self.__dict__['dtype'] = '<i4'
		else:
