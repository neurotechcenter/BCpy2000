...
class BciGenericSource(Core.BciCore):
	...
	def _Preflight(self, in_signal_props):
		class PreflightErr(Exception): pass
		
		super(BciGenericSource, self)._Preflight(in_signal_props)      # superclass
		
		self.out_signal_props = self.in_signal_props
		self.out_signal_props['ChannelLabels'] = list(map(str, list(range(1,1+int(self.params['SourceCh'])))))
		self.out_signal_props['ElementLabels'] = list(map(str, list(range(1,1+int(self.params['SampleBlockSize'])))))
		...
		if self.out_signal_dim != sigdims:
			raise PreflightErr("Source module's Preflight hook may not change the packet dimensions")
	...
	def _Initialize(self, in_signal_dim, out_signal_dim):
		if hasattr(self, '_save_wd'): # we have to do this because the filewriter seems to change directory between our Preflight call and now
			os.chdir(self._save_wd)
			delattr(self, '_save_wd')
		super(BciGenericSource, self)._Initialize(in_signal_dim, out_signal_dim)    # superclass
		self._lasttime = 0
	...
	def _Process(self, in_signal):
		...
		self._foundation_uses_string_encoding = isinstance(in_signal, str)
		if self.warp == 0.0:
			self._paused = True
			print("paused at packet %d because self.warp is 0: call self.step() or self.cont()" % self.packet_count)
			while self.warp == 0.0 and self._paused: time.sleep(0.001)
		else:
			...
			if t > deadline:
				if self.since('run', t)['packets'] > 1 and self.warp == 1.0:
					...
					if firsttime: print("WARNING: late packets")
				deadline = t
			sleeptime = min(deadline-5.0, self._lasttime + 0.9 * wait)
			while self.prectime() < sleeptime: time.sleep(0.001)
			while self.prectime() < deadline: pass
	...
	def cont(self, warp=None):
		...
		works.
		"""###
		if warp == None: warp = float(self.params['WarpFactor'])
		self.warp = warp
	def step(self):
		"""
	...
	def Process(self, in_signal):
		...
		return 100.0 * numpy.matrix(numpy.random.rand(*in_signal.shape)*2.0 - 1.0)
