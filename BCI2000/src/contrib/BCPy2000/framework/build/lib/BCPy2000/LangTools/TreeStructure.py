...
def unpacknode(x, ind=0):
	node = trienode()
	nchildren,node.parent,node.count = map(int, struct.unpack(packedfmt_header, x[ind:ind+packedsize_header]))
...
class trie:
	...
	def dist(self, node, normalize=False):
		distrib = {}
		if isinstance(node, basestring): node = self.retrieve(node)
		if node == None: return distrib
		if isinstance(node, int): node = self.nodes[node]
		for letter,ind in node.children.items():
	...
	def kncount(self, string, prefixes=None):
		"""
			N_1+ ( .  string )
		"""###
		if prefixes==None: prefixes = self.dist('').keys()
	...
	def kndist(self, string):
		...
		prefixes = self.dist('').keys()
		for w in d.keys(): d[w] = self.kncount(string+w, prefixes)
		return d
