...
class BciGenericSource(Core.BciCore):
	...
	def _Preflight(self, in_signal_props):
		...
		self.out_signal_props['ChannelLabels'] = map(str, range(1,1+int(self.params['SourceCh'])))
		self.out_signal_props['ElementLabels'] = map(str, range(1,1+int(self.params['SampleBlockSize'])))
