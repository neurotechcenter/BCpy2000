...
def dprime(*cc, **kwargs):
	...
	if len(kwargs):
		...
		if len(kwargs): raise TypeError("unexpected keyword argument %s" % kwargs.keys()[0])
...
def kview(K, y=None):
	...
	if y == None:
		...
		order = zip(*sorted([(yi,i) for i,yi in enumerate(y)]))[1]
...
class predictor(sstruct):
	...
	def doublecv(self, x=None, K=None, y=None, istrain=None, istest=None, outerfg=None, **innerkwargs):
		...
		if folded and not isequal(outerfg.ids, range(len(y))): raise ValueError("outerfg has the wrong ids for this dataset")
	...
	def varyhyper(self, *pargs, **kwargs):
		...
		for arg in pargs:
			...
			elif hasattr(arg, 'items'): arg = arg.items()
	...
	def plotd(self, hold=False, drawnow=True):
		...
		for hh in h.values(): hh.set_clim(numpy.array([-1,+1]) * max(abs(numpy.array([z.min(), z.max()]))))
...
class lda2class(predictor):
	...
	def training(self, x, y):
		...
		if self.hyper.shrinkage == 'optimal':
			for k,v in c.items(): v /= n[k] # mean of centered per-exemplar outer products (biased ML estimator of cov)
			for i,yi in enumerate(y):
				xi = x[i] - m[yi]
				zi = numpy.outer(xi.flat, xi.flat)
				z[yi] = z.get(yi, 0.0) + ((zi - c[yi]).flatten() ** 2).sum()
			for k,v in c.items(): v *= float(n[k]) / float(n[k]-1) # correct to unbiased estimate 
			for k in z: z[k] = z[k] * float(n[k]) / float(n[k] - 1) ** 3.0    # divide by (n-1) for unbiased estimate of z variances, then multiply by n/(n-1)^2
		else:
			for k,v in c.items(): v /= n[k]-1 # straight to unbiased estimate 

		dm = (m[+1] - m[-1])
		
		for k,cov in c.items():
...
class foldguide(object):
	...
	def __init__(self, ids=None, labels=None, folds=None, ntrain=None, ntest=None, balance=True, randomseed='auto'):
		...
		if isinstance(ids, int): ids = range(ids)
		self.n = len(ids)
		self.ids = list(ids)
	...
	def check(self):
		...
		overlap_tr['average'] = numpy.mean(overlap_tr.values())
...
class experiment(sstruct):
	...
	def _shortdesc(self, x, delim=', '):
		...
		def shortstr(v):
			...
			elif isinstance(v, basestring): cand2 = '[string length %d]' % len(v)
			else: cand2 = '[...]'
	...
	def __getitem__(self, ind):
		if isinstance(ind, tuple):
			...
			e = sstruct()
			s = self._ind2sub(ind)
		elif isinstance(ind, basestring):
...
def overlapping(nsamples=None, windowlength=None, nwindows=None, overlap=None):
	...
	nnones = sum([v==None for k,v in original.items()])
	...
	t0 = range(skipfront, maxstart+1, step)
