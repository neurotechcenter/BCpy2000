...
def bci2000path( filename ):
	if isinstance( filename, basestring ) and isinstance( BCI2000_ROOT_DIR, basestring ) and not os.path.exists( filename ) and os.path.exists( os.path.join( BCI2000_ROOT_DIR, filename ) ):
	...
	if os.path.isdir(tmpdir):
		...
		for file in files:
			if keep: print "os.remove(r'%s')" % file
				...
				try: os.remove(file)
				except Exception, err: sys.stderr.write( "failed to remove %s:\n    %s\n" % ( file, str( err ) ) )
...
def megs(x, rec=0):
	...
	def siz(x, rec):
		...
		return m
	if isinstance(x, dict):                   return siz(x, rec) + megs(x.items(), rec)
