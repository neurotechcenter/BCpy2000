...
class Decoder(object):
	...
	def prior(self):
		return pdict(zip(self.labels, numpy.exp(self.logprior).flat))
	...
	def evolution(self, n=None, flash=False, log=False, **kwargs): # &&&
		...
		if 1:     ax = pylab.subplot(1,sp,1); pylab.imshow(ll, vmin=vmin, vmax=vmax, interpolation='nearest', aspect='auto'); ax.set_yticks(range(self.N)); ax.set_yticklabels(lab) ; ax.set_title('%s likelihood' % kwargs.get('title', ''))
		if 1:     ax = pylab.subplot(1,sp,2); pylab.imshow(lp, vmin=vmin, vmax=vmax, interpolation='nearest', aspect='auto'); ax.set_yticks(range(self.N)); ax.set_yticklabels(lab) ; ax.set_title('%s posterior' % kwargs.get('title', ''))
		if flash: ax = pylab.subplot(1,sp,3); pylab.imshow(c,  vmin=0,    vmax=1,    interpolation='nearest', aspect='auto'); ax.set_yticks(range(self.N)); ax.set_yticklabels(lab) ; ax.set_title('%s flash' % kwargs.get('title', ''))
		return ll,lp
...
def LoadLanguageModel(filename, **kwargs):
	...
	obj = cPickle.load(openfn(filename, 'rb'))
...
default_alphabet36 = u'abcdefghijklmnopqrstuvwxyz -.,;:"\'?!'
default_alphabet   = u' -.,;:"\'?!()0123456789abcdefghijklmnopqrstuvwxyz'
german_alphabet    = u' -.,;:"\'?!()0123456789abcdefghijklmnopqrstuvwxyzäöüß'
...
class LanguageModel(object):
	...
	def save(self, filestem='', compress=True):
		...
		cPickle.dump(self, openfn(filename, 'wb'), -1)
...
class MKN(LanguageModel):
	...
	def CountNGrams(self):
		Q = dict([(
			n,
			dict([  (i,0) for i in range(1, self.maxc+2)+['more']  ])
		) for n in range(1, self.maxn+1)])
		
		t = self.prefixtree.nodes
		stack = [t[0].children.values()]
		...
		while len(stack):
			...
			Q[n][c] += 1
			if n < self.maxn: stack.append(node.children.values())
		return Q
	...
	def N(self, d, criterion, exact=False):
		...
		"""###
		if exact: return sum([c == criterion for wi,c in d.items()])
		else:     return sum([c >= criterion for wi,c in d.items()])
	...
	def kndist(self, string):
		...
		else: successors = d.children.keys()
		gnirts = string[::-1]
		p = {}
