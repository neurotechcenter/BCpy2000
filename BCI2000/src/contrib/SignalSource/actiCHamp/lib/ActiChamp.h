...
#define CHAMP_ERR_MONITORING	(-4)	/*!< either module not present for 20% of a second or data rate 10% above nominal rate  */
#define CHAMP_ERR_SUPPORT		(-5)	/*!< Function is not supported */
...
#define CHAMP_PLL_FREQ_MAX ((int)27.0e6)
/*----------------------------------------------------------------------------*/
/* Data Types */

#pragma pack(push, 1) /* override default packing for structures */

/* Version info about ActiChamp rev. 1..4 */
typedef struct {
  DWORD Dll;
  DWORD Driver;
  DWORD Cypress;
...
/* Version info about ActiChamp rev. 5..6 */
typedef struct {
  DWORD Dll;
  DWORD Driver;
  DWORD Cypress;
  DWORD McFpga; /* media converter fpga */
  DWORD Msp430;
  DWORD CbFpga; /* carrier board fpga */
} t_champVersionExt;
...
/*----------------------------------------------------------------------------*/
/*!
  Function to get extended device settings for acquisition
\param hDevice - device handle
...
/*----------------------------------------------------------------------------*/
/*!
  Function to get device voltages
\param hDevice - device handle
...
/*----------------------------------------------------------------------------*/
/*!
  Function to get extended device voltages
\param hDevice - device handle
\param Settings - pointer to buffer for results
\return - error code
*/
CHAMP_API int WINAPI champGetVoltagesExt(HANDLE hDevice, t_champVoltagesExt *Voltages);
/*----------------------------------------------------------------------------*/
/*!
...
/*----------------------------------------------------------------------------*/
CHAMP_API int WINAPI champGetPll(void *hDevice, t_champPll *Pll);
CHAMP_API int WINAPI champSetPll(void *hDevice, t_champPll *Pll);
#endif /* ModID_XXX_H */
