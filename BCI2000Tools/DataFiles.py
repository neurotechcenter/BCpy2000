# -*- coding: utf-8 -*-
#
#   $Id: DataFiles.py 5752 2018-05-09 16:38:35Z jhill $
#
...
def dump(f, *pargs, **kwargs):
    ...
    * Special variable 'discard':
    ...
    """  ###
    if len(pargs):
        if len(pargs) > 1 or len(kwargs) > 0:
            raise TypeError(
                "use only keyword arguments (exception: one single string, not followed by keyword args)"
            )
        command = pargs[0]
        if not isinstance(command, str):
            raise TypeError("subcommand must be a string")
        if command == "flush":
            if hasattr(f, "close"):
                f.close()
            f = None
...
def start(f):
    if isinstance(f, str):
        fn = f
        if not os.path.isfile(f) and os.path.isfile(f + ".gz"):
            f += ".gz"
        if f.lower().endswith(".gz"):
            openfn = gzip.open
        else:
    ...
    while 1:
        ...
        try:
            ...
            if k not in dims:
                dims[k] = list(v.shape)
                dtypes[k] = v.dtype
            if tuple(v.shape) != tuple(dims[k]):
                raise ValueError("dimensions of variable %s are not consistent" % k)
...
def load(f, catdim=0, maxcount=None, discard=True, ignore=None, return_details=None):
    ...
    hdrs = [hdr for hdr in hdrs if hdr]
    if not hdrs:
        return {}
    ...
    for h in hdrs[1:]:
        ...
        for k, v in list(h["indices"].items()):
            if len(v) == 1 and len(hdrs[0]["indices"][k]) == 1:
                h["indices"][
                    k
                ] = (
                    []
                )  # if a variable appears only once in the current file, and also only once in the first file, ignore it in the current file
    # for i,h in enumerate(hdrs):
    # 	print "hdrs[%d]"%i
    # 	for k,v in h['indices'].items(): print "%s: %d" %(k,len(v))
    for h in hdrs:
        for k, v in list(h["indices"].items()):
            if ignore != None and k in ignore:
                continue
            sofar = totals.get(k, 0)
            if maxcount == None:
                trim = 0
            else:
                trim = max(0, sofar + len(v) - maxcount)
            if trim:
                h["indices"][k] = v = v[:-trim]
    ...
    for hdr in hdrs:
        ...
        if "x" in indices:
            print("loading %d exemplars from %s" % (len(indices["x"]), fn))
        else:
        ...
        while 1:
            ...
            if naccepted < len(indices[k]):
                expected = indices[k][naccepted]
            else:
                expected = None
            if nseen != expected:
            ...
            if a == None:
                ...
                v = juggledim(v, catdim)
                if k not in subs:
                    subs[k] = [slice(None) for s in v.shape]
                subs[k][catdim] = filled[k]
                if len(subs[k]) == 1:
                    content[k][
                        subs[k]
                    ] = v  # either a bug or a very weird feature in numpy requires this special case
                else:
                    content[k][
                        subs[k]
                    ].flat = v.flat  # though this *should* do the job either way
            filled[k] += 1
        # print "    " + ', '.join([('%s (x %d)' % (k,v)) for k,v in sorted(accepted_from_this_file.items())])
        if isinstance(return_details, dict):
            return_details[fn] = accepted_from_this_file
        if isinstance(return_details, list):
            return_details.append((fn, accepted_from_this_file))

    if len(discards):
        print(
            "discarded variables: "
            + ", ".join(
                [("%s (%d instances)" % (k, v)) for k, v in sorted(discards.items())]
            )
        )
...
def dumpmethod(self, *pargs, **kwargs):
    ...
    * Special usage: self.dump('flush')
    ...
    if self._dumpfilename == None:
        ...
        self._dumpfilename = os.path.realpath(
            os.path.splitext(self.data_file)[0] + ".pk"
        )  # NB: using .pk.gz sometimes results in 'CRC check failed' exceptions while trying to load after a trigger-trap error occurred

    dh = self._dumphistory[self._dumpfilename] = self._dumphistory.get(
        self._dumpfilename, {}
    )

    arg = self._dumpfilehandle
    if arg == None:
        arg = self._dumpfilename
    arg = dump(arg, *pargs, **kwargs)
    self._dumpfilehandle = arg
    if arg == None:
        self._dumpfilename = None  # a returned None instead of a filehandle is the sign that the file has been closed
...
def loadmethod(self, f=None, catdim=0):
    if f == None:
        ...
        if not os.path.isfile(f) and os.path.isfile(f + ".gz"):
            f += ".gz"
    elif isinstance(f, str) and not os.path.isfile(f) and len(os.path.split(f)[0]) == 0:
        f = os.path.join(self.data_dir, f)
    if isinstance(f, str):
        f = os.path.realpath(f)
    if f != None and f == getattr(self, "_dumpfilename", None):
        try:
            self._dumpfilehandle.close()
        except:
            print(
            ...
            self._dumpfilehandle = self._dumpfilename = None
...
try:
    try:
        from BCI2000PythonCore import BciCore
    except ImportError:
        from BCPy2000.Generic import BciCore
except ImportError:
    pass
else:
