# -*- coding: utf-8 -*-
#
#   $Id: PlaybackSourceModule.py 3445 2011-08-08 15:46:38Z jhill $
#
...
try:
    from BCI2000PythonSource import BciGenericSource, EndUserError
except ImportError:
    from BCPy2000.GenericSource import BciGenericSource, EndUserError

#################################################################
#################################################################


class PlaybackError(EndUserError):
    pass
...
class BciSource(BciGenericSource):
    ...
    def Preflight(self, inprop):
        ...
        mappings = dict(list(os.environ.items()) + [("DATA", dataroot)])
        sub = lambda x: mappings.get(x.group(1), x.group())
        if sys.platform.lower().startswith("win"):
            # (2) make %HOME% or $HOME equivalent to %USERPROFILE% or %HOMEDRIVE%%HOMEPATH%
            if not "HOME" in mappings:
                mappings["HOME"] = mappings.get(
                    "USERPROFILE", mappings.get("HOMEDRIVE") + mappings.get("HOMEPATH")
                )
        ...
        if nch != pbnch:
            raise PlaybackError(
                "mismatch between number of channels in SourceCh parameter (%d) and playback file (%d)"
                % (nch, pbnch)
            )
        ...
        if fs != pbfs:
            raise PlaybackError(
                "mismatch between sampling rate in SamplingRate parameter (%gHz) and playback file (%gHz)"
                % (fs, pbfs)
            )
        ...
        if isinstance(ts, list):
            if ts.matrixlabels()[1] != ["Channel", "Frequency", "Amplitude"]:
                raise EndUserError(
                    "TestSignals matrix must have 3 columns labelled Channel, Frequency and Amplitude"
                )
            for x in ts:
                if x == ["", "", ""]:
                    continue
                ...
                try:
                    ...
                    if chan in self.params["ChannelNames"]:
                        chan = self.params["ChannelNames"].index(chan)
                    else:
                        raise EndUserError(
                            "unrecognized Channel name '%s' in TestSignals matrix"
                            % chan
                        )
                else:
                    if chan < 1 or chan > nch or chan != round(chan):
                        raise EndUserError(
                            "invalid Channel index %d in TestSignals matrix" % chan
                        )
                    chan = int(chan + 0.5) - 1
                try:
                    freq = float(freq)
                    ...
                    raise EndUserError(
                        "invalid Frequency value '%s' in TestSignals matrix" % freq
                    )
                try:
                    amp = float(amp)
                except:
                    raise EndUserError(
                        "invalid Amplitude value '%s' in TestSignals matrix" % amp
                    )
    ...
    def Process(self, sig):
        ...
        for chan, freq, amp in self.testsig:
            ...
            newsig[chan, :] += amp * numpy.sin(
                2.0 * numpy.pi * freq * sig[0, : newsig.shape[1]]
            )
