...
def ClassifyERPs(
    ...
    file_inventory = []
    d = DataFiles.load(
        featurefiles, catdim=0, maxcount=maxcount, return_details=file_inventory
    )
    if isinstance(folds, str) and folds.lower() in [
        "lofo",
        "loro",
        "leave on run out",
        "leave one file out",
    ...
    if classes != None:
        ...
        if discarded:
            print(
                "discarding %d trials that are outside the requested classes %s"
    ...
    if uy.size != 2:
        raise ValueError(
            "expected 2 classes in dataset, found %d : %s" % (uy.size, str(uy))
        )
    for uyi in uy:
    ...
    cov, trchvar = SigTools.spcov(
        x=x, y=y, balance=False, return_trchvar=True
    )  # NB: symwhitenkern would not be able to balance

    starttime = time.time()

    chlower = [ch.lower() for ch in d["channels"]]
    if keepchan in [None, (), "", []]:
        if isinstance(rmchan, str):
            rmchan = rmchan.split()
        if isinstance(rmchan_usualsuspects, str):
        ...
        if isinstance(keepchan, str):
            keepchan = keepchan.split()
    ...
    if len(notfound):
        print(
            "WARNING: could not find channel%s %s\n"
            % ({1: ""}.get(len(notfound), "s"), ", ".join(notfound))
        )
    removed = [ch for removing, ch in zip(unwanted, d["channels"]) if removing]
    if len(removed):
        print(
            "removed %d channel%s (%s)"
            % (len(removed), {1: ""}.get(len(removed), "s"), ", ".join(removed))
        )
    print(
        "classification will be based on %d channel%s"
        % (sum(wanted), {1: ""}.get(sum(wanted), "s"))
    )
    ...
    if not isinstance(C, (tuple, list, numpy.ndarray, type(None))):
        C = [C]
    ...
    if c != None:
        c.hyper.C = list(C)
    if gamma == None:
        c.hyper.kernel.func = SigTools.linkern
    else:
        c.varyhyper(
            {
    ...
    if gamma == None:
        ...
        Ps = SigTools.svd(
            SigTools.shrinkcov(cov, copy=True, gamma=chosen.kernel.gamma)
        ).isqrtm
    ...
    if save or select:
        if not isinstance(save, str):
            save = featurefiles
            if isinstance(save, (tuple, list)):
                save = save[-1]
            if save.lower().endswith(".gz"):
                save = save[:-3]
            if save.lower().endswith(".pk"):
                save = save[:-3]
        ...
        Parameters.Param(
            description, Name="SignalProcessingDescription", Section="PythonSig"
        ).append_to(save)
        if select:
            if not isinstance(select, str):
                select = "ChosenWeights.prm"
    ...
    **kwargs
    ...
    try:
        ...
        if return_full:
            ...
            out = (c.loss.train, len(c.input.x))
    return out
...
def PlotSCD(files="*.pk"):
    ...
    SigTools.imagesc(
        r * numpy.abs(r),
        y=d["channels"],
        x=SigTools.samples2msec(list(range(r.shape[1])), d["fs"]),
        aspect="auto",
        balance=0.0,
        colorbar=True,
    )
...
def PlotTrials(files="*.pk", channel="Cz"):
    ...
    SigTools.imagesc(
        v,
        x=SigTools.samples2msec(list(range(v.shape[1])), d["fs"]),
        aspect="auto",
        balance=0.0,
        colorbar=True,
    )
