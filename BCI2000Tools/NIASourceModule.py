# -*- coding: utf-8 -*-
#
#   $Id: NIASourceModule.py 3445 2011-08-08 15:46:38Z jhill $
#
...
class NIA_Interface(object):
    ...
    def __init__(
        ...
        for bus in buses:
            for device in bus.devices:
                if (
                    device.descriptor.idVendor == self.VENDOR_ID
                    and device.descriptor.idProduct == self.PRODUCT_ID
                ):
                    found = True
                    break
            if found:
        ...
        if hasattr(usb, "get_driver_np"):
            ...
            if name != "":
                ...
                usb.detach_kernel_driver_np(
                    self.handle, self.interface
                )  # self.handle.detachKernelDriver(0)
                # self.handle.detachKernelDriver(1)
        usb.set_configuration(
            self.handle, self.config.bConfigurationValue
    ...
    def read(self):
        ...
        usb.interrupt_read(
            self.handle, self.ENDPOINT1, self.INPUT_BUFFER, self.TIME_OUT
        )
...
class BciSource(BciGenericSource):
    ...
    def Initialize(self, indim, outdim):
        ...
        if order > 0 and cutoff > 0.0:
            self.filter = SigTools.causalfilter(
                freq_hz=cutoff, samplingfreq_hz=self.hwfs, order=order, type="lowpass"
            )  # , method=SigTools.firdesign)
        else:
            self.filter = None
        self.dsind = numpy.linspace(
            0.0,
    ...
    def Process(self, sig):
        ...
        sig[nch:, :] = self.packet_count * packetsize + numpy.array(
            list(range(packetsize)), ndmin=2, dtype="float"
        ).repeat(sig.shape[0] - nch, axis=0)

        # low-pass
        if self.filter != None:
            x = self.filter.apply(x, axis=1)
        # downsample
    ...
    def Listen(self, mythread):
        ...
        while not mythread.read("stop"):
            ...
            while (
                len(data) < nsamp
            ):  # was set to perform self.Points=25 reads at a time---why 25, we don't know exactly
                time.sleep(0.001)
