...
def read_bracketed(x, brackets=("{ ", " }")):
    ...
    for pos, c in enumerate(x):
        if x[pos : pos + len(opener)] == opener:
            counter += 1
        if x[pos : pos + len(closer)] == closer:
            counter -= 1
        if counter == 0:
            break
    ...
    if stripped.endswith(closer):
        stripped = stripped[: -len(closer)].rstrip()
...
def split_elements(x):
    ...
    while len(x):
        ...
        if len(deeper):
            elements.append(split_elements(deeper))
        elif len(xi):
            elements.append(unescape(xi))
    return elements
...
def convert(val, type, name="", check_nesting=False, mode="Value", opts=()):
    ...
    if type.endswith("list"):
        val = [
            convert(x, type[: -len("list")], name, check_nesting=True, mode=mode)
            for x in val
        ]
        ...
        val = numpy.array(
            [
                [
                    convert(
                        x, type[: -len("matrix")], name, check_nesting=True, mode=mode
                    )
                    for x in row
                ]
                for row in val
        ...
        try:
            numval = floatval = float(valstr)
        except:
            pass
        try:
            numval = intval = int(valstr)
        except:
            if floatval != None and float(int(floatval)) == floatval:
                intval = int(floatval)

        if type == "float":
            if floatval != None:
                numval = floatval
            elif mode == "NumericValue" and "q" not in opts:
                print(
                    (
                        'WARNING: invalid float value "%s" in parameter%s'
                        % (str(val), namestr)
                    )
                )
        if type == "int":
            if intval != None:
                ...
                print(
                    (
                        'WARNING: invalid int value "%s" in parameter%s'
                        % (str(val), namestr)
                    )
                )
        if type == "string":
        ...
        if mode == "Value":
            pass
        elif mode == "NumericValue":
            val = numval
        elif mode == "Units":
            val = units
        elif mode == "BaseUnits":
            val = base_units
        elif mode == "ScaledValue":
            val = numval * scaling
        elif mode == "StringValue":
            val = str(val)

    return val


enummatch = re.compile(
    "^\s*(?P<val>.*?)[\s:]+(?P<key>[0-9]+)[\s,]+"
)  # match backwards from the end of a comment
...
def make_param(x, parent=None, opts=()):
    ...
    if parent == None:
        ...
        if len(location):
            d["Filter"] = ":".join(location)
    type = chomp(x, "Type", d, lower=True)
    if parent == None:
        name = chomp(x, "Name", d)
        if not name.endswith("="):
            raise ValueError("parameter name %s must end with =" % name)
    ...
    if islist:
        ...
        try:
            length = nExpected = int(length)
        except:
            raise ValueError('invalid length "%s" in parameter %s' % (length, name))
    elif ismatrix:
        rows = chomp(x, "rows", name=name)
        cols = chomp(x, "cols", name=name)
        if isinstance(rows, list):
            d["RowLabels"], rows = rows, len(rows)
        if isinstance(cols, list):
            d["ColumnLabels"], cols = cols, len(cols)
        try:
            rows = int(rows)
        except:
            raise ValueError(
                'invalid number of rows "%s" in parameter %s' % (rows, name)
            )
        try:
            cols = int(cols)
        except:
            raise ValueError(
                'invalid number of columns "%s" in parameter %s' % (cols, name)
            )
        nExpected = rows * cols
        if rows == 0 and cols > 0 and "ColumnLabels" not in d:
            d["ColumnLabels"] = cols

    if len(x) < nExpected:
        raise ValueError(
            "expected %d elements in parameter %s - found only %d"
            % (nExpected, name, len(x))
        )
...
class Param(object):
    ...
    def Rows(self):
        ...
        if type.endswith("matrix"):
            return numpy.asarray(self.Value).shape[0]
        else:
            return 1
    ...
    def Columns(self):
        ...
        if type.endswith("list"):
            ...
            if self.Rows() == 0:
                if isinstance(self.ColumnLabels, int):
                    return self.ColumnLabels
                elif self.ColumnLabels != None:
    ...
    def merge(self, newer):
        ...
        if out.RowLabels != None:
            if isinstance(out.RowLabels, int):
                oldRows = out.RowLabels
            else:
                oldRows = len(out.RowLabels)
            if oldRows != newer.Rows():
                out.RowLabels = None
        if out.ColumnLabels != None:
            if isinstance(out.ColumnLabels, int):
                oldColumns = out.ColumnLabels
            else:
                oldColumns = len(out.ColumnLabels)
            if oldColumns != newer.Columns():
                out.ColumnLabels = None

        if out.RowLabels == None or newer.RowLabels != None:
            out.RowLabels = newer.RowLabels
        if out.ColumnLabels == None or newer.ColumnLabels != None:
            out.ColumnLabels = newer.ColumnLabels
        return make_param(out)

    def determine_type(self, type=None):
        if type == None:
            type = self.Type
        if type not in (None, "auto"):
            return type.lower()
        ...
        if isinstance(x, numpy.ndarray):
            if len(x.shape) == 0:
                x = x.flat[0]
            elif len(x.shape) in (1, 2):
                x = x.tolist()
            else:
                raise ValueError("don't know how to deal with >2-D arrays")
        ...
        if isinstance(x, (tuple, list)):
            if False not in [isinstance(xi, int) for xi in x]:
                return "intlist"
            if False not in [isinstance(xi, (int, float)) for xi in x]:
                return "floatlist"
            if False not in [isinstance(xi, (int, float, str)) for xi in x]:
                return "list"
            if False not in [isinstance(xi, (tuple, list)) for xi in x]:
                return "matrix"
    ...
    def format(self, field, type=None, verbosity=1, width=0, widths_out=None):
        def getval(self, type):
            val = self.Value
            if type.endswith("list"):
                if numpy.asarray(val).ndim != 1:
                    val = numpy.asarray(val).flatten()
            elif type.endswith("matrix"):
                val = numpy.asarray(val)
                while len(val.shape) < 2:
                    val = numpy.expand_dims(val, -1)
            return val
        ...
        if field == "Location":
            if verbosity < 0:
                ...
                txt = escape(section)
                extra = ""
                if len(subsection) or len(filter):
                    extra += ":" + escape(subsection)
                if len(filter):
                    extra += ":" + escape(filter)
            ...
            if type.endswith("list"):
                ...
                nrows = self.Rows()
                if self.RowLabels == None or isinstance(self.RowLabels, int):
                    txt = str(nrows)
                    just = "numeric"
                elif len(self.RowLabels) != nrows:
                    raise ValueError(
                        "wrong number of row labels (got %d, expected %d)"
                        % (len(self.RowLabels), nrows)
                    )
                else:
                    txt = " ".join([escape(lab) for lab in self.RowLabels])
                    if verbosity == 0 and len(txt) > 10:
                        txt = "..."
                    txt = "{ " + txt + " }"
            else:
                txt = ""
            ...
            if type.endswith("matrix"):
                ncols = self.Columns()
                if self.ColumnLabels == None or isinstance(self.ColumnLabels, int):
                    txt = str(ncols)
                    just = "numeric"
                elif len(self.ColumnLabels) != ncols:
                    raise ValueError(
                        "wrong number of column labels (got %d, expected %d)"
                        % (len(self.ColumnLabels), ncols)
                    )
                else:
            ...
            if verbosity == 0:
                ...
                txt = self.Comment.strip()
            type = self.determine_type(type)
            if type == "bool" and not txt.endswith("(boolean)"):
                txt += " (boolean)"
            elif (
                type in ("int", "intlist", "enum")
                and isinstance(self.Enumeration, dict)
                and len(self.Enumeration)
                and not txt.endswith("(enumeration)")
            ):
                if verbosity > 0:
                    txt += " " + (
                        ", ".join(
                            [
                                str(k) + ": " + str(v)
                                for k, v in sorted(self.Enumeration.items())
                            ]
                        )
                    )
                if verbosity >= 0:
                    txt += " (enumeration)"
            if verbosity >= 0 and (mark_empty_comments or len(txt)):
                txt = "// " + txt
    ...
    def report(self, verbosity=1, widths_out=None, widths_in=None, reprtxt=False):
        ...
        for field in fields.split():
            if widths_in != None:
                width = widths_in.get(field, 0)
            txt += self.format(
                field=field,
                type=type,
                verbosity=verbosity,
                width=width,
                widths_out=widths_out,
            )
        if verbosity < 0:
            txt = "{ " + txt.strip() + " }"
        if reprtxt:
            txt = "<%s object at 0x%08X>: %s" % (self.__class__.__name__, id(self), txt)
        return txt

    def write_to(self, file, append=False, **kwargs):
        return write_to(self.report(**kwargs), file=file, append=append)

    def append_to(self, file, **kwargs):
        return write_to(self.report(**kwargs), file=file, append=True)
    ...
    def __getitem__(self, sub):
        def conv(self, i, x):  # helper function for converting subscripts
            if isinstance(x, (tuple, list)):
                if i == None:
                    return x.__class__([conv(self, i, xi) for i, xi in enumerate(x)])
                else:
                    return x.__class__([conv(self, i, xi) for xi in x])
            if i == None:
                i = 0
            if isinstance(x, slice):
                return slice(
                    conv(self, i, x.start), conv(self, i, x.stop), conv(self, i, x.step)
                )
            if i == 0:
                lab = self.RowLabels
                labname = "row"
            elif i == 1:
                lab = self.ColumnLabels
                labname = "column"
            else:
                raise TypeError("too many subscripts")
            if isinstance(lab, int):
                lab = None
            if isinstance(x, int):
                if x < 0:
                    x += numpy.asarray(self.Value).shape[i]
                return x
            if not isinstance(x, str):
                return x
            if lab == None or x not in lab:
                raise ValueError("%s label '%s' not found" % (labname, x))
            return lab.index(x)

        sub = conv(self, None, sub)
        if not hasattr(sub, "__len__") or len(sub) == 1:
            result = numpy.asarray(self.Value).__getitem__(sub)
        elif len(sub) == 2:
            result = numpy.asarray(numpy.asmatrix(self.Value).__getitem__(sub))
        else:
            result = numpy.asarray(self.Value).__getitem__(sub)
        if isinstance(result, numpy.ndarray):
            if len(result.shape) < 1:
                result = result.tolist()
            elif len(result.shape) == 1 and result.dtype.kind not in "fib":
                result = result.tolist()
        return result
    ...
    def __cmp_location__(self, other):
        ...
        def f(x):
            return "%s:%s: %s" % (s(x.Section), s(x.Subsection), s(x.Name))
...
def read_parameter_lines(f):
    ...
    if len(line) < 1000 and "HeaderLen=" in line:
        ...
        while (
            f.tell() < stop
            and len(line)
            and not line.startswith("[ Parameter Definition ]")
        ):
            line = f.readline()
...
class ParamList(list):
    def __init__(self, x=(), opts=()):
        ...
        for xi in x:
            if xi == None:
                continue
            if isinstance(xi, str) and len(xi.strip()) == 0:
                continue
            if not isinstance(xi, Param):
                xi = make_param(str(xi), opts=opts)
            self.append(xi)

    def dbreport(self):
        [p.dbreport() for p in self if hasattr(p, "dbreport")]
    ...
    def collate(self, merge_meta=False):
        ...
        while i < len(self):
            ...
            if name == None:
                ...
                if merge_meta:
                    self[d[name]] = self[d[name]].merge(self.pop(i))
                else:
                    self[d[name]] = self.pop(i)
            else:
                d[name] = i
                i += 1
    ...
    def electrodes(self):
        if "ChannelNames" not in self:
            raise KeyError("no ChannelNames parameter")
        try:
            from SigTools.Electrodes import ChannelSet
        except ImportError:
            from BCPy2000.SigTools.Electrodes import ChannelSet
        c = ChannelSet(self.ChannelNames.Value)
        if (
            "ReferenceChannelName" in self
        ):  # despite the singular name, should be a list
    ...
    def __names(self):
        return [
            p.Name for p in self if getattr(p, "Name", None) != None
        ]  # a handy helper for __getitem__, __getattr__ and __contains__

    def _getAttributeNames(self):
        return self.__names()  # for IPython

    def __contains__(self, x):
        return x in self.__names()

    def __getitem__(self, sub):  # de-reference Param elements dict-like, by their .Name
        if isinstance(sub, str):
            names = self.__names()
            if sub in names:
                sub = len(self) - 1 - names[::-1].index(sub)
            else:
                raise KeyError(sub)
    ...
    def __getattr__(
        self, name
    ):  # de-reference Param elements lazily as virtual attributes of the list
        try:
            return self[name]
        except KeyError:
...
def make_bciprm(*pargs, **kwargs):
    ...
    while len(pl):
        ...
        if isinstance(p, str) and p.startswith(">>") and len(p.lstrip(" >")):
            a = ParamList(out, opts=opts)
            a.collate(merge_meta=True)
            a.append_to(p.lstrip(" >"))
        elif isinstance(p, str) and p.startswith(">") and len(p.lstrip(" >")):
            a = ParamList(out, opts=opts)
            a.collate(merge_meta=True)
            a.write_to(p.lstrip(" >"))
        elif isinstance(p, str) and p.startswith("-"):
            opts.append(p.lstrip("-"))
        elif (islegalkey(p) or p in [">", ">>"]) and key == None:
            key = p
            continue
        elif isfilename(p) and key == None:
            out += ParamList(p, opts=opts)
        elif isinstance(p, dict):
            if key != None:
                raise ValueError(
                    'did not expect dict object to follow parameter name "%s"'
                    % str(key)
                )
            ...
            if key != None:
                raise ValueError(
                    'did not expect Param object to follow parameter name "%s"'
                    % str(key)
                )
            out.append(p)
            ...
            raise ValueError(
                "%s argument received without a preceding parameter name"
    ...
    if key != None:
        raise ValueError(
            'extraneous key "%s" was passed without accompanying value' % key
        )
...
def read_bcidate(x, fmt=None):
    ...
    def tryparse(x):
        if not isinstance(x, str):
            return None
        try:
            return time.strptime(" ".join(x.split()[1:]), "%b %d %H:%M:%S %Y")
        except:
            pass
        try:
            return time.strptime(x.lower().replace("t", " "), "%Y-%m-%d %H:%M:%S")
        except:
            pass
    ...
    if d == None:
        if isinstance(x, str):
            x = ParamList(x)
        if isinstance(x, ParamList):
            x = getattr(x, "StorageTime", None)
        if isinstance(x, Param):
            x = x.Value
        if isinstance(x, (tuple, list)):
            return [read_bcidate(xi, fmt=fmt) for xi in x]
        if isinstance(x, (float, time.struct_time)):
            d = x
        else:
            d = tryparse(x)
    if fmt != None:
...
def test(filename, zarg=False, width=2000):
    ...
    if zarg:
        ...
        zarg = ""
    os.system(
        "diff -wyW%d original.prm reported.prm | grep '|'%s | less -S" % (width, zarg)
    )
