    ...
    def __getattr__(self, key):
        if key in self.__dict__:
            ...
            raise AttributeError(
                "'%s' object has no attribute or item '%s'"
...
def ReadMatFile(file):
    ...
    while ReadArray(file, container):
        pass
    return container
...
def ReadArray(file, container, index=None):
    ...
    if arrayStartFlag == None:
        return False
    if arrayStartFlag != (miMATRIX,):
        raise ValueError("array header started with unexpected code")
    nBytes, flagType, flagLengthBytes, arrayType, flagPadding = chomp(
        file, "<LLLLL"
    )  # NB: nBytes cannot be relied upon for structs and cell arrays written by bci_stream2mat
    if (flagType, flagLengthBytes, flagPadding) != (miUINT32, 8, 0):
        raise ValueError("array header contained unexpected data")
    dimType, dimLengthBytes, nRows, nCols = chomp(file, "<LLll")
    numel = nRows * nCols
    shape = [nRows, nCols]
    if (dimType, dimLengthBytes) != (miINT32, 8):
        raise ValueError("array dimension info contained unexpected data")
    data = [[None for i in range(nCols)] for i in range(nRows)]
    nameType, nameLengthBytes = chomp(file, "<LL")
    if nameType != miINT8:
        raise ValueError("array name had unexpected name type")
    if nameLengthBytes:
        name = file.read(nameLengthBytes)
        while file.tell() % matPadding:
            file.read(1)
        if index == None:
            index = name
    if index == None:
        raise ValueError("no name recorded in file, and none supplied in call")
    ...
    if arrayType == mxSTRUCT_CLASS:
        ...
        if fieldNameDataFlags != ((4 << 16) | miINT32):
            raise ValueError("unexpected data flags for struct fieldname record")
        if fieldNameDataType != miINT8:
            raise ValueError("unexpected data type for struct fieldname record")
        if fieldNameDataSize % fieldNameLength:
            raise ValueError(
                "fieldname data block size is not an integer number of fieldNameLengths"
            )
        fields = [
            chomp(file, str(fieldNameLength) + "s")[0].strip("\x00")
            for i in range(fieldNameDataSize / fieldNameLength)
        ]
        ...
        if dataType != miUTF16:
            raise ValueError("unexpected data type for payload in character array")
        if dataLengthBytes != numel * 2:
            raise ValueError("unexpected size for payload in character array")
        for iCol in range(nCols):
            for iRow in range(nRows):
        ...
        if dataType != expectedDataType:
            raise ValueError(
                "payload for numeric array of type %d has unexpected data type %d"
                % (arrayType, dataType)
            )
        if moreFlags not in [4, 0]:
            raise ValueError("unexpected flags in numeric array")
        if not (moreFlags & 4):
            (dataSizeBytes,) = chomp(file, "<L")
            if dataSizeBytes != numel * struct.calcsize(dataElementFormat):
                raise ValueError(
                    "payload for %s numeric array of type %d has unexpected block size %d bytes"
                    % ("x".join([str(x) for x in shape]), arrayType, dataSizeBytes)
                )
        for iCol in range(nCols):
            for iRow in range(nRows):
