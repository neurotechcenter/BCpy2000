# -*- coding: utf-8 -*-
#
#   $Id: SpellerTools.py 3445 2011-08-08 15:46:38Z jhill $
#
...
def GetCode(listofcodes, key):
    ...
    if len(c) < 1:
        raise IndexError("Name='%s' is not found in codebook list" % key)
    if len(c) > 1:
        raise IndexError("Name='%s' is not unique in codebook list" % key)
    return c[0]
...
def ConstructRowOfCodebooksParam(c, pages=None, order=None, name=None):
    ...
    if pages == None:
        pages = c["Pages"]
    if isinstance(pages, (tuple, list, str)) and len(pages) == 0:
        pages = "%"
    ...
    if c["RandomizeTime"] not in [0, 1] and len(c["RandomizeTime"]) != m.shape[1]:
        raise IndexError(
            "RandomizeTime failed because the number of columns in codebook and parameter do not match"
        )

    if c["RandomizeSpace"] not in [0, 1] and len(c["RandomizeSpace"]) != m.shape[0]:
        raise IndexError(
            "RandomizeSpace failed because the number of rows in codebook and parameter do not match"
        )

    mstr = ConstructMatrixSubparam(
        m
    )  # m could be a space/newline-delimited string,  or an actual numpy array of numeric 1's and 0's
...
def ConstructCodebooksParam(*coderows):
    if len(coderows) == 1 and isinstance(coderows[0], (tuple, list)):
        coderows = coderows[0]
    names = [x.partition(" ")[0] for x in coderows]
    if len(set(names)) < len(names):
        raise ValueError("duplicate names in codebook")
    return (
        "EncDec:Encoding matrix Codebooks= "
        + str(len(coderows))
        + " { "
        + " ".join([z[0] for z in CodebooksParamHeadings])
        + " } "
        + " ".join(coderows)
    )
...
class CodebooksItem(sstruct):
    def __init__(self, d, pages=1, reorder=None):
        ...
        if isinstance(self.Matrix, str):
            self.Matrix = MultilineStringToNumpyArray(self.Matrix)
        self.Pages = pages
        self._reorder_fields(
            ("Name", "Pages", "RandomizeTime", "RandomizeSpace", "Matrix"), 0
        )
        if reorder != None:
...
class CodebooksParam(list):
    def __init__(self, arg):
        ...
        for i in range(len(self)):
            x = self[i]
            # x = copy.deepcopy(x)
            if not isinstance(x, CodebooksItem):
                x = CodebooksItem(x)
            self[i] = x

    def __delitem__(self, ind):
        if isinstance(ind, str):
            ind = [x.Name for x in self].index(ind)
        return list.__delitem__(self, ind)

    def __getitem__(self, ind):
        if isinstance(ind, str):
            return GetCode(self, ind)
        else:
...
def ReorderSubsetOfGrid(grid, page, reordering):
    ...
    colHeader = cols = [
        x
        for x in grid[grid.find("{") + 1 : grid.find("}")].split(" ")
        if not len(x) == 0
    ]
...
def ParseGrid(grid):
    ...
    while True:
        try:
            ...
            ValueError
        ):  # @@@ ValueError is too generic to be relied upon---can happen for all sorts of reasons---explicitly throw your own class of Exception if you need to (see TrieError in trie.py)
...
class GridItem:
    def __init__(self, label, posX, posY, command="", scale=None):
        ...
        if scale == None:
            scale = 0.1 * round(10.0 / len(label) ** 0.5)
        self.scale = scale
    ...
    def __str__(self):
        ...
        label = str(self.label).encode("unicode_escape")
        s = s[:-1] + " : %s  %2.1f  (% 5.1f,% 5.1f) %s >" % (
            label.rjust(5),
            self.scale,
            self.posY,
            self.posX,
...
class GridPage(list):
    ...
    def stringRep(self, pagenum, xtransform=None, ytransform=None):
        if xtransform == None:
            ...
            xmult, xadd = xtransform
        if ytransform == None:
            ymult, yadd = 1.0, 0.0
        else:
            ymult, yadd = ytransform
    ...
    def __repr__(self):
        ...
        s += (
            " (%d items): [\n    " % len(self)
            + ",\n    ".join([str(x).encode("raw_unicode_escape") for x in self])
            + "\n]"
        )
        return s
...
class GridParam(list):
    def __init__(self, arg):
        ...
        if isinstance(arg, GridPage) or not isinstance(arg, (tuple, list)):
            arg = [arg]
        list.__init__(self, arg)
        for i in range(len(self)):
            x = self[i]
            if not isinstance(x, GridPage):
                x = GridPage(x)
            self[i] = x
    ...
    def __str__(self):
        x, y = numpy.array(
            [(item.posX, item.posY) for item in reduce(list.__add__, self)]
        ).T
        ...
        intro = (
            "PythonApp:Stimulus matrix Grid= "
            + str(sum([len(x) for x in self]))
            + " { "
        )
        paramHeadings = " ".join([escape(x) for x, y in GridParamHeadings])
        params = " } " + "   ".join(
            [self[i].stringRep(i + 1, xtransform, ytransform) for i in range(len(self))]
        )
