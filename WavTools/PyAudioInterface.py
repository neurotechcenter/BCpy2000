...
try:
    ...
    print(
        __name__,
        "module failed to import pyaudio: playback and recording will not work",
    )
...
def grab_interface(verbose=False):
    ...
    p = pyaudio.PyAudio()
    if verbose:
        print(repr(p), "has been initialized")
    return p
...
def release_interface(p, verbose=False):
    ...
    if p == None:
        return
    if verbose:
        print(repr(p), "is about to be terminated")
    p.terminate()
...
def record(seconds=None, nchan=None, fs=None, bits=None, w=None):
    ...
    if w == None:
        if not seconds:
            raise ValueError("please specify number of seconds for which to record")
        if not nchan:
            nchan = 2
        if not fs:
            fs = 44100
...
class recorder(Background.ongoing):
    def __init__(
        ...
        if w == None:
            ...
            w = Base.wav(fs=fs, bits=bits, nchan=nchan)
            if seconds:
                w.y = Base.silence(round(seconds * w.fs), nchan)
        else:
            if not seconds:
                seconds = w.duration()
            if not nchan:
    ...
    def record(self, bg=True, seconds=None, nchan=None):
        ...
        if seconds == None and self.seconds == None:
            raise ValueError(
                "please specify a number of seconds to record, either in the constructor or the record() call"
            )
        self.go(bg=bg)
        return self
    ...
    def core(self, seconds=None, nchan=None):
        ...
        if nchan != self.wav.channels() or seconds != self.wav.duration():
            print(
                "resizing recorder's internal wav object from (%s channels x %s seconds) to (%s channels x %s seconds)"
                % (
                    str(self.wav.channels()),
                    str(self.wav.duration()),
                    str(nchan),
                    str(seconds),
                )
            )
        ...
        while self.keepgoing and not self.timedout():
            aimfor = (
                float(self.packets_recorded + 1) * float(fs) / float(self.packetRateHz)
            )
    ...
    def handle_data(self, packet):
        ...
        if nsamp:
            if across_samples == 0:
                self.wav.y[start:stop, :] = packet[:nsamp, :]
            elif across_samples == 1:
                self.wav.y[:, start:stop] = packet[:, :nsamp]
            else:
                raise RuntimeError("well, which dimension is the sample dimension?")
...
class player(Background.ongoing):
    def __init__(self, w=None, verbose=False, buffersize=None, dev=None):
        ...
        if buffersize == None:
            ...
            if platform.system().lower() == "darwin":
                buffersize = 450  # TODO:  maybe there's a way of querying the interface as to what this should be?
        ...
        if self.verbose:
            print(repr(self), "has been initialized")
        self.open(w, dev=dev)

    def __del__(self):
        self.close()
        if self.verbose:
            print(repr(self), "is about to be destroyed")
    ...
    def ready(self, w):
        ...
        rate = w.fs
        return (
            self.stream._format == format
            and self.stream._channels == nchan
            and self.stream._rate == rate
        )
    ...
    def openstream(self, w=None, buffersize=None, dev=None):
        ...
        if self.verbose:
            print(repr(self.stream), "has been opened")

    def closestream(self):
        stopped = self.stop(wait=0.5)
        if not stopped:
            raise IOError("cannot close PyAudio stream")
        if self.stream != None:
            if self.verbose:
                print(repr(self.stream), "is about to be closed")
            self.stream.close()
        self.stream = None

    def play(
        self, repeats=1, bg=True, w=None, data=None, timeout=None, vol=None, pan=None
    ):
        """
        ...
        if Base.channels(data) != Base.channels(w):
            raise ValueError(
                "data array and wav object have mismatched numbers of channels"
            )
    ...
    def unpause(self):
        if not self.speed:
            self.speed = getattr(self, "previous_speed", 1)
    ...
    def core(self, repeats=1, w=None, data=None):
        ...
        while repeats != 0:
            ...
            while start < nsamp:
                ...
                if jump:
                    start += Base.msec2samples(jump * 1000, fs)
                if start <= 0:
                    start = 0
                if start >= nsamp:
                    break
                while True:
                    if not self.keepgoing or self.timedout():
                        self.keepgoing = False
                        break
                    if self.speed:
                ...
                if speed == 1.0:
                    ...
                    if repeats == 1:
                        stop = min(float(nsamp), stop)
                    xi = numpy.linspace(
                        start=start, stop=stop, endpoint=False, num=towrite
                    )
                    xi %= float(nsamp)
                    dd = Base.interpsamples(data, xi)

                vols = float(self.vol) * Base.panhelper(
                    self.pan, nchan=dd, norm=self.norm
                )
                ...
                if (
                    not preplay_done
                    and self.preplay != None
                    and self.preplay["func"] != None
                ):
                    self.preplay["func"](
                        *self.preplay["pargs"], **self.preplay["kwargs"]
                    )
                    preplay_done = True
                if len(self.timestamps["before"]) < 100:
                    self.timestamps["before"].append(prectime())
                self.stream.write(raw)
                if len(self.timestamps["after"]) < 100:
                    self.timestamps["after"].append(prectime())
                start = stop
            if not self.keepgoing:
                break
            repeats -= 1
        ...
        towrite = self.stream.get_write_available()
        bytes_per_frame = (
            self.interface.get_sample_size(self.format) * self.stream._channels
        )
        if towrite > 0:
            self.stream.write("\0" * towrite * bytes_per_frame)
        while self.stream.get_write_available() < self.stream._frames_per_buffer:
            sleep(0.001)
        sleep(
            float(self.stream._frames_per_buffer) / fs
    ...
    def playing():
        def fget(self):
            return self.going or self.__playing
    ...
    def get_timestamp(self):
        return self.timestamps[
            "play"
        ]  # the problem with timestamps['before'] and timestamps['after']
    ...
    def fade(self, duration=1.0, target=0.0, delay=0.0):
        ...
        self.vol = Modulate.modulator(
            timefunc=Modulate.ramp,
            start=self.vol,
            stop=target,
            duration=duration,
            delay=delay,
...
def wavplay(self, *pargs, **kwargs):
    ...
    if tempplayer:
        playerinstance = player(w=self, verbose=True)
    kwargs["w"] = self
    kwargs["bg"] = False
    t = playerinstance.play(*pargs, **kwargs)
    if tempplayer:
        del playerinstance  # TODO: check whether we need to allow time for the buffer to empty before the stream gets destroyed by the garbage collector
