...
if platform.system().lower() == "windows":
    ...
    def SetMasterVolume(volume, channel=None):
        ...
        if ret != 0:
            raise WindowsError("Error %d while setting master volume" % ret)
    ...
    def GetMasterVolume(channel=None):
        ...
        if ret != 0:
            raise WindowsError("Error %d while getting mixer control details" % ret)
    ...
    if platform.win32_ver()[0].lower().startswith("xp"):
        try:
            GetMasterVolume()  #  control ID=0 is correct on some XP machines but fails with an exception on others
        except:
            SPEAKER_LINE_FADER_ID = 1  #  control ID=1 is correct on some XP machines
    else:
        SPEAKER_LINE_FADER_ID = 2  # control ID=2 seems to be correct on some Vista/Win7 machines (whereas 0 throws an error and 1 is, extremely inconveniently, the mute control)

if SetMasterVolume == None:
    print(
        __name__,
        "module could not find an implementation for SetMasterVolume---only supported for Win32 at the moment",
    )
