...
try:
    try:
        ...
        from BCPy2000.PrecisionTiming import prectime
except:
    print(
        __name__,
        "module failed to import prectime: using less precise time.time() for timestamps",
    )
...
class ongoing:
    ...
    def __init__(self, pargs=(), kwargs={}, timeout=None):
        ...
        self.thread = None
        if ongoing.verbose:
            print(repr(self), "has been initialized as an ongoing instance")

    def __del__(self):
        self.stop(wait=1.0)
        if ongoing.verbose:
            print(repr(self), "is about to be destroyed as an ongoing instance")

    def go(self, bg=True):
        class DoubleGoError(Exception):
            pass

        self.thread = None
        if bg:
            if self.going:
                raise DoubleGoError("already running")
            if ongoing.verbose:
                print(repr(self), "is launching a thread")
            self.thread = _thread.start_new_thread(self.go, (), {"bg": False})
            return
        if ongoing.verbose:
            print(repr(self), "is starting")
        self.going = True
        self.keepgoing = True
        self.starttime = prectime()
        try:
            self.core(*self.pargs, **self.kwargs)
    ...
    def stop(self, wait=None):
        ...
        if not self.going:
            return True
        if ongoing.verbose:
            print(repr(self), "is waiting to finish")
        deadline = prectime() + 1000.0 * wait
        while self.going and (wait < 0.0 or prectime() < deadline):
            sleep(0.001)
        if ongoing.verbose and self.going:
            print(repr(self), "has not finished (timed out after %g seconds)" % wait)
