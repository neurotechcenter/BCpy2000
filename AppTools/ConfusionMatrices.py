...
class confusion_matrix:
    def __init__(self, bci, classes=None, styles=("", "%d/%d", "%d%%")):
        if classes == None:
            classes = list(range(len(bci.introwavs) + 1))
        ...
        totalprops = {
            "font_name": font_name,
            "font_size": int(font_size * 0.65),
            "color": (1.0, 1.0, 1.0),
            "anchor": "center",
        }
        cellprops = {
            "font_name": font_name,
            "font_size": int(font_size * 1.0),
            "anchor": "center",
        }
        ...
        for ystep in range(0, nc + 2):
            ...
            for xstep in range(0, nc + 2):
                ...
                if xstep == 0 and yin:
                    stim = VisualStimuli.Text(
                        position=(x, y),
                        text=str(self.classes[ystep - 1]),
                        anchor="right",
                        **labelprops
                    )
                    ...
                    if yin:
                        self.rowtotaltxt.append(stim)
                    elif xin:
                        self.coltotaltxt.append(stim)
                    else:
                        self.grandtotaltxt = stim
                elif xin and yin:
                    if xstep == ystep:
                        color = (0, 0.7, 0)
                    else:
                        color = (0.4, 0.2, 0.2)
                    stim = VisualStimuli.Text(
                        position=(x, y), text="", color=color, **cellprops
                    )
                    self.celltxt[-1].append(stim)
                if stim != None:
                    if callable(getattr(bci, "stimulus", None)):
                        bci.stimulus(
                            "confusionmatrix_" + str(xstep) + "_" + str(ystep), stim
                        )
                    else:
    ...
    def update(self, trueclass, predictedclass):
        if not trueclass in self.classes:
            return
        if not predictedclass in self.classes:
            return
        self.matrix[
            self.classes.index(trueclass), self.classes.index(predictedclass)
        ] += 1
        self.redraw()
    ...
    def redraw(self):
        ...
        for i in range(nc):
            settext(
                self.coltotaltxt[i], self.displaystr(a[i, i], ct[i], self.styles[0])
            )
            settext(
                self.rowtotaltxt[i], self.displaystr(a[i, i], rt[i], self.styles[1])
            )
            settext(
                self.grandtotaltxt, self.displaystr(a.trace(), rt.sum(), self.styles[2])
            )
            for j in range(nc):
                settext(self.celltxt[i][j], "%d" % a[i, j])

    def displaystr(self, nright, ntotal, style="%d/%d"):
        if "/" in style:
            return style % (nright, ntotal)
        elif len(style) == 0:
            return " "
        elif ntotal == 0:
            return "-"
        else:
            return style % round(100.0 * float(nright) / float(ntotal))
