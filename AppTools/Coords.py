...
__all__ = [
    ...
    "Box",
    "union",
    "union_of_boxes",  # synonymous
    "intersection",
    "intersection_of_boxes",  # synonymous
]
...
class Point(list):
    def __init__(
        self, position=(), x=None, y=None, z=None, step=None, box=None, default_val=0.0
    ):
    ...
    def _x_fset(self, val):
        self[0] = val

    x = property(
        _x_fget, _x_fset, doc="the x coordinate: the first element of the sequence"
    )
    # @apply
    ...
    def _y_fset(self, val):
        self[1] = val

    y = property(
        _y_fget, _y_fset, doc="the y coordinate: the second element of the sequence"
    )
    # @apply
    ...
    def _z_fset(self, val):
        self[2] = val

    z = property(
        _z_fget, _z_fset, doc="the z coordinate: the third element of the sequence"
    )
    # @apply
    ...
    def _A_fset(self, val):
        ...
        if val.size != max(val.shape):
            raise ValueError(
                "arrays for assignment to a Point may only have non-singular extent in one dimension"
            )
        it = val.flat
        for i in range(len(it)):
            self[i] = it[i]
        for i in range(len(it), len(self)):
            self[i] = self._default

    A = property(
        _A_fget, _A_fset, doc="get or set the Point's content as a numpy array"
    )

    def _M_fget(self):
        import numpy

        return numpy.matrix(self).T
    ...
    M = property(
        _M_fget, _M_fset, doc="get or set the Point's content as a numpy matrix"
    )

    def up(self):
        self.y += self.__step.y
        return self
    ...
    def __setitem__(self, pos, val):
        ...
        val = float(val)
        if pos >= len(self):
            list.__iadd__(self, [self._default] * (pos - len(self) + 1))
    ...
    def map(self, f):
        return self.__class__([f(x) for x in self], step=self.__step, box=self.__box)
    ...
    def set(self, *pargs, **kwargs):
        for k, v in list(pargs) + list(kwargs.items()):
            setattr(self, k, v)
        return self
    ...
    def __add__(self, other):
        return self.copy().__op(operator.add, other)

    def __sub__(self, other):
        return self.copy().__op(operator.sub, other)

    def __mul__(self, other):
        return self.copy().__op(operator.mul, other)

    def __div__(self, other):
        return self.copy().__op(operator.div, other)

    def __radd__(self, other):
        return self.copy().__op(operator.add, other)

    def __rsub__(self, other):
        return self.copy().__op(operator.sub, other, swap=True)

    def __rmul__(self, other):
        return self.copy().__op(operator.mul, other)

    def __rdiv__(self, other):
        return self.copy().__op(operator.div, other, swap=True)
...
class Box(object):
    ...
    def __init__(self, arg=None, **k):
        ...
        if (
            not Box._alignment_dic
        ):  # class dict is empty .. lets initialize it once for all instances
        ...
        for key in keys:
            if arg != None:
                if key == "internal" and hasattr(arg, "internal_initialized"):
                    gotit = arg.internal_initialized()
                else:
                    gotit = hasattr(arg, key)
                if gotit:
                    setattr(self, key, getattr(arg, key))
            if key in k:
                setattr(self, key, k.pop(key))
    ...
    def copy(self, cls=None):
        ...
        if self.internal_initialized():
            b.internal = self.internal.copy()
        return b
    ...
    def __repr__(self):
        ...
        if anchorstr == None:
            anchorstr = "custom"
        s += "\n    " + "size %s, %s anchor point at (%s), %s" % (
            " x ".join(map(str, self.size)),
            anchorstr.replace(" ", "-"),
            ", ".join(map(str, self.position)),
            (self.sticky and "sticky" or "non-sticky"),
        )
        if self.__internal != None:
            ib = self.__internal
            ndi = len(ib)
            if ndi == 2:
                s += "\n    " + "internal coordinates X [%+g %+g] by Y [%+g %+g]" % (
                    ib.__getlim(0) + ib.__getlim(1)
                )
            else:
                s += (
                    "\n    "
                    ...
                    + " by ".join(
                        ["[%+g %+g]" % ib.__getlim(i) for i in range(len(ib))]
                    )
                )
        return s

    def __len__(self):
        return max(len(self.__size), len(self.__position), len(self.__anchor))

    def __equalize_len(self):
        n = len(self)
        for i in range(len(self.__size), n):
            self.__size[i] = self.__size[i]
        for i in range(len(self.__position), n):
            self.__position[i] = self.__position[i]
        for i in range(len(self.__anchor), n):
            self.__anchor[i] = self.__anchor[i]

    def __i2n(self, val, dim=None):  # internal to normalized
        if isinstance(val, (tuple, list)):
            if dim == None:
                return val.__class__([self.__i2n(val[i], i) for i in range(len(val))])
            else:
                val = val[dim]
        if self.__internal == None:
            return (float(val) - -1.0) / 2.0
        else:
            lo, hi = self.__internal.__getlim(dim)
            return (float(val) - lo) / (hi - lo)

    def __n2i(self, val, dim=None):  # normalized to internal
        if isinstance(val, (tuple, list)):
            if dim == None:
                return val.__class__([self.__n2i(val[i], i) for i in range(len(val))])
            else:
                val = val[dim]
        if self.__internal == None:
            return -1.0 + float(val) * 2.0
        else:
            lo, hi = self.__internal.__getlim(dim)
            return lo + float(val) * (hi - lo)

    def __getlim(self, dim):
        size, position, anchor = (
            self.__size[dim],
            self.__position[dim],
            self.__get_anchor_coords()[dim],
        )
    ...
    def _lims_fget(self):
        return tuple([self.__getlim(i) for i in range(len(self))])

    def _lims_fset(self, val):
        n = len(val)
        for i in range(n):
            self.__setlim(i, val[i])
        del self.__size[n:]
        del self.__position[n:]
        del self.__anchor[n:]

    lims = property(
        _lims_fget,
        _lims_fset,
        doc="for each dimension, a tuple of (left/low/near, right/high/far) limits, expressed in external coordinates",
    )
    # @apply
    ...
    rect = property(
        _rect_fget,
        _rect_fset,
        doc="the (left, bottom, right, top) external coordinates of the Box",
    )

    def _left_fget(self):
        return self.__getlim(0)[0]

    def _left_fset(self, val):
        self.__setlim(0, (val, None))

    left = property(
        _left_fget,
        _left_fset,
        doc="the position of the left edge of the Box in external coordinates",
    )
    # @apply

    def _right_fget(self):
        return self.__getlim(0)[1]

    def _right_fset(self, val):
        self.__setlim(0, (None, val))

    right = property(
        _right_fget,
        _right_fset,
        doc="the position of the right edge of the Box in external coordinates",
    )
    # @apply

    def _bottom_fget(self):
        return self.__getlim(1)[0]

    def _bottom_fset(self, val):
        self.__setlim(1, (val, None))

    bottom = property(
        _bottom_fget,
        _bottom_fset,
        doc="the position of the lower edge of the Box in external coordinates",
    )
    # @apply

    def _top_fget(self):
        return self.__getlim(1)[1]

    def _top_fset(self, val):
        self.__setlim(1, (None, val))

    top = property(
        _top_fget,
        _top_fset,
        doc="the position of the upper edge of the Box in external coordinates",
    )
    # @apply

    def _near_fget(self):
        return self.__getlim(2)[0]

    def _near_fset(self, val):
        self.__setlim(2, (val, None))

    near = property(
        _near_fget,
        _near_fset,
        doc="the position of the near edge of the Box in external coordinates",
    )
    # @apply

    def _far_fget(self):
        return self.__getlim(2)[1]

    def _far_fset(self, val):
        self.__setlim(2, (None, val))

    far = property(
        _far_fget,
        _far_fset,
        doc="the position of the far edge of the Box in external coordinates",
    )
    # @apply

    def _size_fget(self):
        a = self.__size
        n, nmax = len(a), len(self)
        if n < nmax:
            list.__iadd__(a, [a._default] * (nmax - n))
        return a
    ...
    size = property(
        _size_fget,
        _size_fset,
        doc="the extent of the Box in each dimension, in external coordinates",
    )
    # @apply

    def _width_fget(self):
        return self.__size.x

    def _width_fset(self, val):
        self.__size.x = val

    width = property(
        _width_fget,
        _width_fset,
        doc="the extent of the Box in the x dimension, in external coordinates",
    )
    # @apply

    def _height_fget(self):
        return self.__size.y

    def _height_fset(self, val):
        self.__size.y = val

    height = property(
        _height_fget,
        _height_fset,
        doc="the extent of the Box in the y dimension, in external coordinates",
    )
    # @apply

    def _depth_fget(self):
        return self.__size.z

    def _depth_fset(self, val):
        self.__size.z = val

    depth = property(
        _depth_fget,
        _depth_fset,
        doc="the extent of the Box in the z dimension, in external coordinates",
    )
    # @apply
    ...
    position = property(
        _position_fget,
        _position_fset,
        doc="the external coordinate of the Box's anchor point",
    )
    # @apply
    ...
    def _x_fset(self, val):
        self.__position.x = val

    x = property(
        _x_fget,
        _x_fset,
        doc="the x coordinate of the Box's anchor point in external coordinates",
    )
    # @apply
    ...
    def _y_fset(self, val):
        self.__position.y = val

    y = property(
        _y_fget,
        _y_fset,
        doc="the y coordinate of the Box's anchor point in external coordinates",
    )
    # @apply
    ...
    def _z_fset(self, val):
        self.__position.z = val

    z = property(
        _z_fget,
        _z_fset,
        doc="the z coordinate of the Box's anchor point in external coordinates",
    )
    ...
    def _anchor_fset(self, val):
        oldorigin = self.__position - self.__size * self.__i2n(
            self.__get_anchor_coords()
        )
        ...
        if self.__sticky:
            neworigin = self.__position - self.__size * self.__i2n(
                self.__get_anchor_coords()
            )
    ...
    def _anchorstr_fget(self):
        return self.__anchorstr

    def _anchorstr_fset(self, val):
        self.anchor = val

    anchorstr = property(
        _anchorstr_fget,
        _anchorstr_fset,
        doc="the box's anchor expressed as a string (or None if there is no particular name for the anchor point)",
    )

    # @apply

    def _sticky_fget(self):
        return self.__sticky

    def _sticky_fset(self, val):
        self.__sticky = bool(val)
    ...
    def _internal_fget(self):
        if self.__internal == None:
            ...
            ib.lims = [(-1.0, 1.0)] * len(
                self
            )  # do not change these: these absolute defaults are also assumed elsewhere
        return self.__internal

    def _internal_fset(self, val):
        self.__internal = val
    ...
    def int2ext(self, obj, attr=None):
        ...
        """  ###
        if (
            not isinstance(obj, (Point, Box))
            and attr != None
            and attr.lower() in ["position", "point"]
        ):
            obj = Point(obj)
        if (
            not isinstance(obj, (Size, Box))
            and attr != None
            and attr.lower() in ["size"]
        ):
            obj = Size(obj)

        if not isinstance(obj, (Point, Size, Box)):
            obj = Point(obj)
