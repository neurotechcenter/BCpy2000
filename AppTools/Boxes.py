...
class Point(list):
    ...
    def __getattr__(self, key):
        if key is "x":
            ...
            return numpy.matrix(self, dtype=numpy.float64).T
        else:
    ...
    def map(self, f):
        return self.__class__([f(x) for x in self], step=self.step, box=self.box)

    def copy(self):
        return self.__class__(self, step=self.step, box=self.box)
    ...
    def __add__(self, other):
        return self.copy().__op__(operator.add, other)

    def __sub__(self, other):
        return self.copy().__op__(operator.sub, other)

    def __mul__(self, other):
        return self.copy().__op__(operator.mul, other)

    def __div__(self, other):
        return self.copy().__op__(operator.div, other)

    def __radd__(self, other):
        return self.copy().__op__(operator.add, other)

    def __rsub__(self, other):
        return self.copy().__op__(operator.sub, other, swap=True)

    def __rmul__(self, other):
        return self.copy().__op__(operator.mul, other)

    def __rdiv__(self, other):
        return self.copy().__op__(operator.div, other, swap=True)
...
class Box(object):
    ...
    def __init__(self, rect=None, **kwargs):
        ...
        if rect is not None and not "rect" in kwargs:
            kwargs["rect"] = rect
        ...
        if init_anchor == "":
            init_anchor = kwargs.get("anchor", "center")
    ...
    def __setattr__(self, key, val):
        ...
        if key in ["size"]:
            self.__dict__["width"], self.__dict__["height"] = self.size
        ...
        if key in ["position", "size", "x", "y", "width", "height"] or (
            key == "anchor" and not self.sticky
        ):
            self.__dict__["rect"] = (
                self.x - self.width * p[0],
                self.y - self.height * p[1],
                self.x + self.width * (1.0 - p[0]),
                self.y + self.height * (1.0 - p[1]),
            )
            (
                self.__dict__["left"],
                self.__dict__["bottom"],
    ...
    def __repr__(self):
        ...
        s += "\n    " + "size %g x %g, %s anchor point at (%g , %g), %s" % (
            self.width,
            self.height,
            str(self.anchor),
            self.x,
    ...
    def shift(self, xy=None, x=None, y=None, proportional=False):
        ...
        """  ###
        if xy != None:
            if not isinstance(xy, tuple) and not isinstance(xy, list):
                xy = (xy, 0)
            x, y = xy
...
def union_of_boxes(*m):
    ...
    """  ###
    if len(m) == 1 and isinstance(m[0], (tuple, list)):
        m = m[0]
    if len(m) == 0:
        return Box(rect=(0, 0, 0, 0))
    mm = m[0].copy()
