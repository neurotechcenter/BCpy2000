...
def PolygonTexture(
    ...
    **kwargs
    ...
    if False not in [isinstance(i, (float, int)) for i in vertices]:
        ...
        vertices = [
            (0.5 + 0.5 * cos(2 * pi * a), 0.5 + 0.5 * sin(2 * pi * a)) for a in vertices
        ]
    ...
    frame.internal.bottom, frame.internal.top = (
        frame.internal.top,
    ...
    return VisualStimuli.ImageStimulus(
        texture=canvas,
        size=size,
        anchor=anchor,
        position=position,
...
def Disc(
    position=(10, 10), size=(1, 1), radius=1, color=(0, 0, 1), anchor="center", **kwargs
):
