...
if platform.system().lower() == "windows":
    try:
        ...
        def monitor(id=0):
            ...
            if id >= len(m):
                raise IndexError(
                    "index %d out of range (%d monitors detected)" % (id, len(m))
                )
            return m[id]
        ...
        def monitor(id=0):
            if not id in [0, -1]:
                raise IndexError(
                    "win32api not available---cannot get information about multiple displays"
                )
            return monitors()[id]
    ...
    print(
        __name__,
        "module does not know how to get information about the number and size of displays",
    )
    ...
    def monitor(id=0):
        if not id in [0, -1]:
            raise IndexError("do not know how to enumerate displays on this system")
        return monitors()[id]
...
def split_33_66(bci, **kwargs):
    ...
    bci.experimenter_panel = init_screen(
        m, **kwargs
    )  # this one also recorded in screen.coords
