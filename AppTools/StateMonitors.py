...
class statemonitor(object):
	def __init__(self, name, position, showtime, params, func=None, pargs=(), kwargs={}):
		...
		self.label = VisualStimuli.Text(position=lpos,anchor='left',on=True,text=name+':',**params)
		self.lastval = 0
		self.time = 0
		self.func = func
		self.pargs = pargs
	...
	def update(self, val, t):
		...
		else: s = self.statename+ ": " + str(val)		
		if self.time == 0 or val != self.lastval: self.lastval,self.time = val,t
		if val and self.showtime: s += '   (%.1f sec)' % (t - self.time)
		p = getattr(self.label, 'parameters', self.label) # .parameters would be for VisionEgg objects
