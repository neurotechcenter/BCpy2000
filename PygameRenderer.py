...
class ImageStimulus(Coords.Box):
	...
	def draw(self, screen, screencoords):
		if 'on' not in self._props or not self._props['on']: return
	...
	def _fget_color(self):  p = self._props; return p['color']
	...
	color =  property(_fget_color, _fset_color, doc='3- or 4-element sequence denoting RGB or RGBA colour')
	


	def _fget_angle(self):  p = self._props; return p['angle']
	...
	def _fget_smooth(self):  p = self._props; return p['smooth']
	...
	smooth = property(_fget_smooth, _fset_smooth, doc='whether or not pygame transformations are smooth')



	def _fget_on(self):  p = self._props; return p['on']
	...
	#@apply
	def _flipx_fget(self):  p = self._props; return p['flipx']
	...
	def _flipy_fget(self):  p = self._props; return p['flipy']
...
class Text(ImageStimulus):
	...
	def _value_fget(self):  p = self._props; return p['value']
	def _value_fset(self, val):
		if isinstance(val, (tuple,list,numpy.ndarray)): val = list(val)
		p = self._props;
		self.__text_changed = p['value'] != val
		p['value'] = val
	value = property(_value_fget, _value_fset, doc='optional list of values for interpolation into text')
		

	def _text_fget(self):  p = self._props; return p['text']
	...
	def _font_name_fget(self):  p = self._props; return p['font_name']
	...
	def _font_size_fget(self):  p = self._props; return p['font_size']
...
def FindFont(fontnames):
	...
	if f == None and sys.platform == 'darwin': # pygame on OSX doesn't seem even to try to find fonts...
		f = (list(filter(os.path.isfile, [os.path.realpath('/Library/Fonts/%s.ttf'%x) for x in fontnames])) + [None])[0]
	return f
